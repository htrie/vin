...
public:
	...
	std::string increase_hue_start() { hsb().hue_start = std::clamp(hsb().hue_start + 5.0f, 0.0f, 360.0f); return std::string("hue_start = ") + std::to_string(hsb().hue_start); }
	std::string decrease_hue_start() { hsb().hue_start = std::clamp(hsb().hue_start - 5.0f, 0.0f, 360.0f); return std::string("hue_start = ") + std::to_string(hsb().hue_start); }

	std::string increase_hue_range() { hsb().hue_range = std::clamp(hsb().hue_range + 5.0f, 0.0f, 360.0f); return std::string("hue_range = ") + std::to_string(hsb().hue_range); }
	std::string decrease_hue_range() { hsb().hue_range = std::clamp(hsb().hue_range - 5.0f, 0.0f, 360.0f); return std::string("hue_range = ") + std::to_string(hsb().hue_range); }

	std::string increase_hue_adjust() { hsb().hue_adjust = std::clamp(hsb().hue_adjust + 5.0f, 0.0f, 360.0f); return std::string("hue_adjust = ") + std::to_string(hsb().hue_adjust); }
	std::string decrease_hue_adjust() { hsb().hue_adjust = std::clamp(hsb().hue_adjust - 5.0f, 0.0f, 360.0f); return std::string("hue_adjust = ") + std::to_string(hsb().hue_adjust); }

	std::string increase_saturation() { hsb().saturation = std::clamp(hsb().saturation + 0.05f, 0.0f, 1.0f); return std::string("saturation = ") + std::to_string(hsb().saturation); }
	std::string decrease_saturation() { hsb().saturation = std::clamp(hsb().saturation - 0.05f, 0.0f, 1.0f); return std::string("saturation = ") + std::to_string(hsb().saturation); }

	std::string increase_brightness() { hsb().brightness = std::clamp(hsb().brightness + 0.05f, 0.0f, 1.0f); return std::string("brightness = ") + std::to_string(hsb().brightness); }
	std::string decrease_brightness() { hsb().brightness = std::clamp(hsb().brightness - 0.05f, 0.0f, 1.0f); return std::string("brightness = ") + std::to_string(hsb().brightness); }
};
