...
namespace Renderer
{
	...
	void RenderGraph::OnResetDevice(Device::IDevice* device, bool gi_enabled, bool use_wide_pixel_format, size_t downscaling_mip, bool dof_enabled, bool smaa_enabled, bool smaa_high)
	{
		...
		const auto backbuffer_width = backbuffer_desc.Width;
		const auto backbuffer_height = backbuffer_desc.Height;

		const simd::vector2_int frame_size = simd::vector2_int(
	...
	void RenderGraph::UpdateBreachPostProcess(const simd::vector3& origin, const float radius)
	{
		post_process.SetBreachSphereInfo(origin, radius);
	}

	void RenderGraph::ToggleBreachEffect(bool enable)
	{
		post_process.ToggleBreachEffect(enable);
	}
	...
	void RenderGraph::RenderDebugInfo(Device::Line & line, const simd::matrix & view_projection)
	{
		screenspace_tracer.RenderWireframe(line, view_projection);
	}
