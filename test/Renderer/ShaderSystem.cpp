...
namespace Shader
{
	...
	public:
		Request(const std::string& address, const Renderer::FragmentLinker::ShaderSource& shader_source, const Text* const text, const std::wstring& uid)
		{
			...
			http_stream.Open(packed_url.c_str(),
				...
				[&](long status_code)
				{
					...
					{
					#if defined(PROFILING)
						LOG_DEBUG(L"[SHADER] Request ended with error status " << status_code);
						LOG_DEBUG(L"[SHADER] Signature: " << StringToWstring(signature));
						LOG_DEBUG(L"[SHADER] URL: " << StringToWstring(url));
					#endif
						bytecode.clear();
					}
				});
		...
		const Device::ShaderData& GetBytecode()
		{
			...
			{
				LOG_DEBUG(L"[SHADER] Request ended with http stream error state");
				bytecode.clear();
			}
		#endif
			return bytecode;
	...
	{
	#if defined(MOBILE)
		return 1; // Have to limit to 1 for now (Curl multi-threading issue on iPhone only). // [TODO] Consider re-implementing HttpStream curl.
