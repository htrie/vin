			...
			public:
				...
				void SetGraph(const Nodes& nodes, const Links& links) override final
				{
					...
					const auto GetLinkEndPoint = [&](const FXGRAPHReader::LinkEndPoint& pt) -> std::optional<unsigned>
					{
						if (const auto stage = GetStage(pt.stage))
						{
							...
							if (found == nodes_map.end())
								throw Resource::Exception(parent->filename) << L"Failed establishing links for node " << StringToWstring(pt.type) << L" at index " << pt.index << " in " << parent->filename;
					...
					for (const auto& link : links)
					{
						...
						const auto src_link_index = (*src_node)->GetEffectNode().GetConnectorIndexByName(link.src.variable, false);
						const auto dst_link_index = (*dst_node)->GetEffectNode().GetConnectorIndexByName(link.dst.variable, true);
