...
namespace Renderer
{
	namespace DrawCalls
	{
		...
		{
			const auto& input_connector_mask = ConnectorInfo(input_link, input_mask);
			Link* link = nullptr;
			for (auto& _link : input_links)
			{
				if (_link.input_info == input_connector_mask)
				{
					auto prev_input_node = _link.node;
					--prev_input_node->num_output_links;
					link = &_link;
					break;
				}
			}

			const auto& output_connector_mask = ConnectorInfo(output_link, output_mask);
			if (link)
			{
			...
			{
				input_links.emplace_back(input_connector_mask, output_connector_mask, &input_node);
				std::sort(input_links.begin(), input_links.end(), [&](const Link& a, const Link& b)
				{
		...
		void EffectGraphNode::AddStageInput(EffectGraphNode& input_node, const unsigned output_link, const std::string& output_mask, const unsigned input_link, const std::string& input_mask)
		{
			...
			{
				stage_links.emplace_back(input_connector_mask, output_connector_mask, &input_node);
				std::sort(stage_links.begin(), stage_links.end(), [&](const Link& a, const Link& b)
				{
					const auto index_a = a.input_info.index;
					const auto index_b = b.input_info.index;
					if (index_a == index_b)
		...
		{
			struct InputLinkInfo
			{
				unsigned output_slot_id;
				EffectGraphNode* input_node;
				InputLinkInfo() {}
				InputLinkInfo(const unsigned output_slot_id, EffectGraphNode* input_node) : output_slot_id(output_slot_id), input_node(input_node) {}
				inline bool operator < (const InputLinkInfo& other) const { return output_slot_id < other.output_slot_id; }
			};
			...
			unsigned node_id = 0;
			if(!static_ref->GetInputConnectors().empty() || static_ref->IsInputType())
			{				
