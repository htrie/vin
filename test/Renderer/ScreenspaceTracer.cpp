...
namespace Renderer
{
	...
	void PhaseLine::RenderWireframe(Device::Line & line, const simd::matrix & view_projection, float vertical_offset, simd::color color) const
	{
	...
	void RenderSource(Device::Line &line, const simd::matrix & view_projection, simd::color color, simd::vector2 pos, float radius, float intensity)
	{
		...
		for (float ang = 0.0f; ang < 2.0f * 3.1415f; ang += step)
		{
			...
			line.DrawTransform(&line_verts[0], 2, &view_projection, color.c());
	...
	void ScreenspaceTracer::RenderWireframe(Device::Line & line, const simd::matrix & view_projection)
	{
		this->shallow_phaseline.RenderWireframe(line, view_projection, -150.0f, 0xff00ff00);
		this->rocky_phaseline.RenderWireframe(line, view_projection, -170.0f, 0xffff0000);
		this->deep_phaseline.RenderWireframe(line, view_projection, -190.0f, 0xff0000ff);

		this->river_bank_phaseline.RenderWireframe(line, view_projection, -210.0f, 0xff00ffff);

		for (auto &source_constraint : water_source_constraints)
		{
			RenderSource(line, view_projection, source_constraint.source > 0.0f ? 0xffff0000 : 0xff00ff00, source_constraint.world_pos, source_constraint.radius, source_constraint.source);
		}
		for (auto &blocking_constraint : water_blocking_constraints)
		{
			RenderSource(line, view_projection, 0xffffffff, blocking_constraint.world_pos, blocking_constraint.radius, 30.0f);
		}
	}
