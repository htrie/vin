...
#else
		Device::SurfaceDesc surface_desc;
		device->GetBackBufferDesc(0, 0, &surface_desc);
		ptCenter.x = surface_desc.Width / 2.0f;
		ptCenter.y = surface_desc.Height / 2.0f;
#endif
...
void BaseCamera::OnRender( Device::Line* line )
{
#if GIZMO_ENABLED
	for( auto& g : camera_gizmos )
		g.OnRender( *this );
...
std::pair<float, float> BaseCamera::GetOrthographicWidthHeight() const
{
	...
	return { ( float )surface_desc.Width * orthographic_scale, ( float )surface_desc.Height * orthographic_scale };
}
...
void MayaCamera::OnRender( Device::Line* line )
{
	RotationCamera::OnRender( line );
