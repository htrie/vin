...
namespace Renderer
{
	...
	#pragma pack( pop )
		...
		public:
			...
			#else
				max_size = (tight_buffers ? 8 : 64) * Memory::MB;
			#endif
			...
			{
				binding_inputs.push_back(Device::BindingInput(Device::IdHash(DrawDataNames::Uniforms)).SetStructuredBuffer(data));
			}
		};
			...
			, imagespace_vertex_declaration(imagespace_vertex_elements)
		{
			...
			breach_sphere_info = simd::vector4(0.f, 0.f, 0.f, 0.f);
		...
		void System::OnCreateDevice(Device::IDevice *_device)
		{
			...
			viewmode_font = Device::Font::Create(_device, 10, 0, FW_NORMAL, 1, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Tahoma");
			viewmode_sprite_batch = Device::SpriteBatch::Create(_device, "Viewmode");
#endif
		}
		...
		void System::OnResetDevice(Device::IDevice *_device)
		{
#ifdef ENABLE_DEBUG_VIEWMODES
			if (viewmode_font)
				viewmode_font->OnResetDevice();
			if (viewmode_sprite_batch)
				viewmode_sprite_batch->OnResetDevice();
#endif
			...
			screen_width = float(backbuffer_desc.Width);
			screen_height = float(backbuffer_desc.Height);
...
#ifdef ENABLE_DEBUG_VIEWMODES
			if (viewmode_font)
				viewmode_font->OnLostDevice();
			if (viewmode_sprite_batch)
				viewmode_sprite_batch->OnLostDevice();
#endif
		...
		void System::FrameMoveBegin(float delta_time, bool render_world)
		{
			...
			Job2::System::Get().Add(Job2::Type::High, { Memory::Tag::Scene, Job2::Profile::EntitiesFlush, [=]() {
		...
		void System::Cull(float delta_time, bool render_world, CubemapTask* cubemap_task)
		{
			...
			GpuParticles::System::Get().FrameMove(delta_time, [&](const AABB& bounding_box)
			{
				if (!render_world)
					return -1.0f;

				auto scene_index = impl->GetSubSceneIndex(bounding_box);
				const auto visibility = ComputeVisibility(bounding_box, impl->GetCameraSubBounds(scene_index).frustum, impl->GetVisibleShadowedLightSubVolumes(scene_index), cubemap_task);
				if (!visibility[(unsigned)Entity::ViewType::Camera])
					return (impl->GetCamera()->GetPosition() - bounding_box.center).len();

				return -1.0f;
			});

			Entity::System::Get().Swap(); // Needs to be done after any commands push. // [TODO] Move to engine global swap.

			auto particle_func = [=](auto& particle, auto& cull)
			{
				cull.bounding_box.Merge(particle.emitter->FrameMove(delta_time));
			};

			auto trail_func = [=](auto& trail, auto& cull)
			{
				cull.bounding_box.Merge(trail.trail->FrameMove(delta_time));
			};

			if (!render_world)
			{
				// Use same particle/trail_func to avoid accumulation when in fullscreen UI (passive skill tree, atlas map).
				auto cull_func = [=](auto& cull, auto& render) {};
				auto render_func = [=](auto& templ, auto& render) { return false; };

				Entity::System::Get().Update(particle_func, trail_func, cull_func, render_func); // Flush commands.
				return;
			}

			auto cull_func = [=](auto& cull, auto& render)
			{
				render.scene_index = impl->GetSubSceneIndex(cull.bounding_box);

				const auto view_bounds = impl->GetCameraSubBounds(render.scene_index);

				render.visibility = (cull.layers & visible_layers) && cull.visible ? ComputeVisibility(cull.bounding_box, view_bounds.frustum, impl->GetVisibleShadowedLightSubVolumes(render.scene_index), cubemap_task) : 0;

				if (render.visibility.test((unsigned)Entity::ViewType::Camera))
				{
					render.lights = CullEntityLights(cull.bounding_box, impl->GetVisibleLights(), impl->GetVisibleLightSubVolumes(render.scene_index));

					if (auto* emitter = render.draw_call->GetEmitter())
						if (!Particles::ParticlesUpdateVisibility(emitter->GetParticles(), impl->GetCamera()->GetViewProjectionMatrix()))
							render.visibility.reset();
				}
				else
				{
					render.lights.clear();
				...
				render.on_screen_size = render.visibility.any() ? 
					ComputeOnScreenSize(cull.bounding_box, view_bounds.tan_fov2, view_bounds.position, screen_height) : -1.0f;
			};

			auto render_func = [=](auto& templ, auto& render)
			{
				...
				const auto& visibility = render.visibility;
				if (visibility.none())
					return false;
				...
				if (render.on_screen_size > 0.0f && 
					(type != DrawCalls::Type::Particle &&
					...
					type != DrawCalls::Type::Doodad))
				{
					for (const auto& a : templ.GetMergedBindingInputs())
					{
						if (a.type == Device::BindingInput::Type::Texture && !a.texture_handle.IsNull())
							a.texture_handle.SetOnScreenSize(render.on_screen_size);
					}
				}

				auto* draw_call = render.draw_call.get();
				draw_call->Update(render.lights, templ.GetBlendMode() >= DrawCalls::BlendModeExtents::BEGIN_COLOUR, render.scene_index);
			...
			Entity::System::Get().Update(particle_func, trail_func, cull_func, render_func);
		}
		...
		Device::UniformInputs System::BuildUniformInputs()
		{
			...
			uniform_inputs.push_back(Device::UniformInput(Device::IdHash(DrawDataNames::BreachSphereInfo)).SetVector(&breach_sphere_info));
		...
		void System::EnableBreachEffect()
		{
			if ( !breach_effect_enabled )
			{
				Renderer::System::Get().ToggleBreachEffect( true );

				breach_effect_enabled = true;
			}
		}

		void System::DisableBreachEffect()
		{
			if ( breach_effect_enabled )
			{
				breach_sphere_info = simd::vector4( 0.f, 0.f, 0.f, 0.f );

				Renderer::System::Get().ToggleBreachEffect( false );

				breach_effect_enabled = false;
			}
		}

		void System::UpdateBreachSphere(simd::vector3 sphere_position, float sphere_radius )
		{
			breach_sphere_info = simd::vector4( sphere_position.x, sphere_position.y, sphere_position.z, sphere_radius );

			Renderer::System::Get().UpdateBreachPostProcess( sphere_position, sphere_radius );
		}

		const simd::vector4& System::GetBreachInfo() const
		{
			return breach_sphere_info;
		}
		...
		void System::DisableBreachClipping()
		{
			...
			if ( !breach_effect_enabled )
			{
				breach_sphere_info = simd::vector4( 0.f, 0.f, 0.f, 20000.f );
				breach_effect_enabled = true;
			}
		}
#endif
