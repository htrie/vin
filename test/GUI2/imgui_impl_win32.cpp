...
static BOOL CALLBACK ImGui_ImplWin32_UpdateMonitors_EnumFunc(HMONITOR monitor, HDC, LPRECT, LPARAM)
{
    ...
    imgui_monitor.DpiScale = ImGui_Platform_GetDpiScaleForMonitor(monitor);
...
float ImGui_Platform_GetDpiScaleForMonitor(void* monitor)
{
...
float ImGui_Platform_GetDpiScaleForHwnd(void* hwnd)
{
    HMONITOR monitor = ::MonitorFromWindow((HWND)hwnd, MONITOR_DEFAULTTONEAREST);
    return ImGui_Platform_GetDpiScaleForMonitor(monitor);
}
...
{
    // Modified from the original backend to preserve focus on the main viewport
...
{
    // Modified from the original backend to preserve focus on the main viewport
    ImGui_ImplWin32_ViewportData* main_data = (ImGui_ImplWin32_ViewportData*)ImGui::GetMainViewport()->PlatformUserData;
    ImGui_ImplWin32_ViewportData* data = (ImGui_ImplWin32_ViewportData*)viewport->PlatformUserData;
    IM_ASSERT(main_data->Hwnd != 0 && data->Hwnd != 0);
    ::SetWindowPos(data->Hwnd, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
    ::SetForegroundWindow(main_data->Hwnd);
    ::SetFocus(main_data->Hwnd);
}
...
static float ImGui_ImplWin32_GetWindowDpiScale(ImGuiViewport* viewport)
{
    ImGui_ImplWin32_ViewportData* vd = (ImGui_ImplWin32_ViewportData*)viewport->PlatformUserData;
    IM_ASSERT(vd->Hwnd != 0);
    return ImGui_Platform_GetDpiScaleForHwnd(vd->Hwnd);
}
...
{
    if (ImGui_Platform_WndProcHandler(hWnd, msg, wParam, lParam))
        return true;
