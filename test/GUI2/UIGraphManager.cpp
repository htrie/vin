...
namespace Device
{
	namespace GUI
	{
		...
		UINode::UIConnectorInfo::UIConnectorInfo(const std::string& name, const GraphType& type) : name(name), type(type)
		{
		}
		...
		float UINode::UIConnectorInfo::OnRender(const UIGraphStyle& style, ImDrawList* draw_list, const float pos_x, const float pos_y, const bool is_input, const std::string& instance_name, UINode* current_node, UINode** src_node, UINode** dst_node, std::string& selected_slot)
		{
		...
		void UINode::AddInputSlot(const std::string& input_name, const GraphType type)
		{
			input_slots.emplace_back(input_name, type);
		}

		void UINode::AddOutputSlot(const std::string& output_name, const GraphType type)
		{
			output_slots.emplace_back(output_name, type);
		}
		...
		bool UINode::IntersectsBox(const ImVec2& min, const ImVec2& max)
		{
		...
		void UINode::OnRender(const UIGraphStyle& style, ImDrawList* draw_list, std::vector<UINode*>& selected_nodes, UINode** input_node, UINode** output_node, std::string& selected_node_input, std::string& selected_node_output, UIParameter** changed_parameter, bool& node_active, bool refresh_selection)
		{
			char buffer[512] = { '\0' };
			char buffer2[512] = { '\0' };
			...
			{	
				...
				slot_y += input_slots[slot_idx].OnRender(style, draw_list, node_rect_min.x, slot_y, true, instance_name, this, input_node, output_node, selected_node_input);
			}
			...
			{
				slot_y += SLOT_PADDING.y;
				slot_y += output_slots[slot_idx].OnRender(style, draw_list, node_rect_max.x, slot_y, false, instance_name, this, output_node, input_node, selected_node_output);
			}
		...
		void UIExtensionPoint::OnRender(const UIGraphStyle& style, ImDrawList* draw_list, std::vector<UINode*>& selected_nodes, UINode** input_node, UINode** output_node, std::string& selected_node_input, std::string& selected_node_output, UIParameter** changed_parameter, bool& node_active, bool refresh_selection)
		{
			char buffer0[512] = { '\0' };
			char buffer1[512] = { '\0' };
			char buffer2[512] = { '\0' };
		...
		void UIGroupRegion::OnRender(const UIGraphStyle& style, ImDrawList* draw_list, std::vector<UINode*>& selected_nodes, UINode** input_node, UINode** output_node, std::string& selected_node_input, std::string& selected_node_output, UIParameter** changed_parameter, bool& node_active, bool refresh_selection)
		{
		...
		bool UIGroupRegion::IntersectsBox(const ImVec2& min, const ImVec2& max)
		{
		...
		void UIGroupRegion::AddChildren(const std::vector<UINode*>& nodes)
		{
			if (nodes.empty())
				return;
		...
		void UIGroupRegion::RemoveChildren(const std::vector<UINode*>& nodes)
		{
			if (nodes.empty())
				return;
		...
		UIGraphManager::UIGraphManager() : 
			m_pOnAddNodeCallback(nullptr),
			m_pOnRemoveNodeCallback(nullptr),
			m_pOnLinkNodesCallback(nullptr),
			m_pOnUnlinkNodesCallback(nullptr),
			m_pOnSplitSlotComponents(nullptr),
			m_pOnParameterChangedCallback(nullptr),
			m_pOnExtensionPointStageChanged(nullptr),
			m_pOnAddNewExtensionPoint(nullptr),
			m_pOnAddNewExtensionPointSlot(nullptr),
			m_pOnRemoveExtensionPointSlot(nullptr),
			m_pOnAddDefaultExtensionPoints(nullptr),
			m_pOnPositionChangedCallback(nullptr),
			m_pOnCopyCallback(nullptr),
			m_pOnPasteCallback(nullptr),
			m_pOnSaveCallback(nullptr),
			m_pOnCustomParameterSet(nullptr),
		...
		{		
			...
			properties_node->OnRender(style, draw_list, selected_nodes, &input_node, &output_node, selected_node_input, selected_node_output, nullptr, node_active, refresh_selection);
			...
			for(auto node_itr = ui_nodes.rbegin(); node_itr != ui_nodes.rend(); ++node_itr)
			{
				...
				node->OnRender(style, draw_list, selected_nodes, &input_node, &output_node, selected_node_input, selected_node_output, &changed_parameter, node_active, refresh_selection);
			...
			if (ImGui::IsMouseReleased(0) && position_changed)
			{
				...
				for (auto itr = selected_nodes.begin(); itr != selected_nodes.end(); ++itr)
				{
					...
					if (!(selected_node->IsGroupRegion() || selected_node->IsExtensionPoint()))
					{
				...
				for (auto group : ungrouped_nodes)
				{
					...
					if (found_group != ui_group_regions.end())
					{
						found_group->second->RemoveChildren(group.second);
				...
				for (auto group : grouped_nodes)
				{
					...
					if (found_group != ui_group_regions.end())
					{
						found_group->second->AddChildren(group.second);
						for (const auto child : group.second)
			...
			if (!ImGui::GetIO().WantTextInput && ImGui::IsKeyPressed(ImGuiKey_B) && !selected_nodes.empty())
			{
		...
		void UIGraphManager::AddUILink(const std::string& input_node, const std::string& output_node, const std::string& input_slot, const std::string& input_slot_mask, const std::string& output_slot, const std::string& output_slot_mask)
		{
			...
			{
				int input_slot_idx = input->get()->GetInputSlotIndex(input_slot);
				int output_slot_idx = output->get()->GetOutputSlotIndex(output_slot);
				...
				ui_links.emplace_back(new UINodeLink(input->get(), output->get(), input_slot + input_mask, output_slot + output_mask));
			}
		}
		...
		bool UIGraphManager::AddOutputExtensionPoint(const unsigned group, const unsigned index, const std::string& slot_name, const GraphType& type, const UIParameter* parameter, const simd::vector2& position)
		{
			...
			if(found_slot == extension_point->input_slots.end())
				extension_point->AddInputSlot(slot_name, type);
		...
		bool UIGraphManager::AddInputExtensionPoint(const unsigned group, const unsigned index, const std::string& slot_name, const GraphType& type, const UIParameter* parameter, const simd::vector2& position)
		{
			...
			if(found_slot == extension_point->output_slots.end())
				extension_point->AddOutputSlot(slot_name, type);
		...
		void UIGraphManager::RemoveLink(const UINodeLink* link)
		{
			...
			int input_slot_idx = input_node.GetInputSlotIndex(input_slot_name);
			int output_slot_idx = output_node.GetOutputSlotIndex(output_slot_name);
		...
		void UIGraphManager::ConnectNodes(UINode* input_node, UINode* output_node, const std::string& selected_node_input, const std::string& selected_node_output)
		{
			...
			int input_slot_idx = input.GetInputSlotIndex(input_slot_name);
			int output_slot_idx = output.GetOutputSlotIndex(output_slot_name);
			assert(input_slot_idx != -1 && output_slot_idx != -1);

			GraphType input_type = UINode::GetMaskTypeHelper(input_slot_mask, input.input_slots[input_slot_idx].type);
			GraphType output_type = UINode::GetMaskTypeHelper(output_slot_mask, output.output_slots[output_slot_idx].type);
			...
			if(m_pOnLinkNodesCallback(input, output, input_slot_idx, input_slot_mask, output_slot_idx, output_slot_mask, m_pCallbackEventUserContext))
			{
				...
				{
					return link->input.node == input_node && link->input.slot == selected_node_input;
				});
				...
				if(found_link != ui_links.end())
				{
					...
					ui_links.emplace_back(new UINodeLink(input_node, output_node, selected_node_input, selected_node_output));
			}
		}
