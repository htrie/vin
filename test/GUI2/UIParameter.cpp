	...
	namespace GUI
	{
		namespace
		{
			struct UnitInfo
			{
				...
				constexpr UnitInfo(const std::string_view& name, const std::string_view& symbol, int group, float scale, float power, float period)
					: name(name)
					, symbol(symbol)
					, group(group)
					, scale(scale)
			...
			{
				constexpr LinearUnit(const std::string_view& name, const std::string_view& symbol, int group, float scale) : UnitInfo(name, symbol, group, scale, 1.0f, -1.0f) {}
			};

			struct AngularUnit : public UnitInfo
			{
				constexpr AngularUnit(const std::string_view& name, const std::string_view& symbol, int group, float scale, float period) : UnitInfo(name, symbol, group, scale, 1.0f, period) {}
			};

			struct ExponentialUnit : public UnitInfo
			{
				constexpr ExponentialUnit(const std::string_view& name, const std::string_view& symbol, int group, float power) : UnitInfo(name, symbol, group, 1.0f, power, -1.0f) {}
			};

			constexpr UnitInfo Units[] = {
				LinearUnit("None",						"",				0, 1.0f								),
				LinearUnit("Percentage",				"%",			0, 0.01f							),
				...
				AngularUnit("Degrees",					"\xC2\xB0",		3, IM_PI / 180.0f,	360.0f			),
				...
				LinearUnit("Millimeters per Second",	"mm/s",			9, 0.1f								),
				LinearUnit("Centimeters per Second",	"cm/s",			9, 1.0f								),
				LinearUnit("Meters per Second",			"m/s",			9, 100.0f							),
				LinearUnit("Inches per Second",			"in/s",			9, 2.54f							),
				LinearUnit("Feet per Second",			"ft/s",			9, 30.48f							),
				LinearUnit("Yards per Second",			"yd/s",			9, 91.44f							),
																						
				ExponentialUnit("RGB",					"",				10, 1.0f							),
				ExponentialUnit("sRGB",					"",				10, 2.2f							),
			};
			...
			inline float ConvertUnit(float value, size_t from, size_t to, bool apply_offset = true)
			{
				...
				if (std::abs(Units[from].power - 1.0f) > 1e-3f || std::abs(Units[to].power - 1.0f) > 1e-3f)
					return std::pow(std::max(0.0f, value), Units[from].power / Units[to].power) * (Units[from].scale / Units[to].scale);
			...
			private:
				...
				{
					bool changed = false;
					const float unit_selection_width = CalcUnitComboWidth(false, layout.unit);

					float v[N] = {};
					for (size_t a = 0; a < N; a++)
						v[a] = ConvertUnit(value[a], layout.unit, unit[0]);

					const auto edit_width = total_width - (unit_selection_width > 0.0f ? unit_selection_width + ImGui::GetStyle().ItemSpacing.x : 0.0f);
					...
					if (changed)
						for (size_t a = 0; a < N; a++)
							value[a] = ConvertUnit(v[a], unit[0], layout.unit);

					if (unit_selection_width > 0.0f && unit[0] < CustomUnit)
					{
						ImGui::SameLine(0.0f, ImGui::GetStyle().ItemSpacing.x);
						ImGui::SetCursorPosX(x + total_width - unit_selection_width);
						ImGui::SetNextItemWidth(unit_selection_width);
						if (UnitCombo(&unit[0], false, layout.unit))
						{
				...
				void LoadNewLayout(const JsonValue& value)
				{
					...
					if (range.IsObject())
					{
						...
						else if (range.HasMember(L"unit"))
						{
				...
				void SaveNewLayout(JsonValue& value, JsonAllocator& allocator) const
				{
					...
					if ((layout.unit < CustomUnit || layout.custom_unit.length() > 0) && layout.unit != NoUnit && layout.mode != LayoutMode::ColorPicker)
					{
