...
#	if DEBUG_GUI_ENABLED && defined(IMGUI_PLATFORM)
		...
		void GUIResourceManager::ReloadFonts()
		{
			...
			if (FileSystem::FileExists(L"Tools/common/gui_font.ttf"))
			{
				...
				io.Fonts->AddFontFromMemoryCompressedTTF( FontData::FontAwesome6_compressed_data, FontData::FontAwesome6_compressed_size, Round< float >( 13 * final_scale ), &merge_config, icon_glyph_range );
			}
			else
			{
				...
				merge_config.OversampleV = 2;
				merge_config.GlyphMinAdvanceX = 15.0f * final_scale;
				merge_config.GlyphOffset.y = 2.0f * final_scale;
				io.Fonts->AddFontFromMemoryCompressedTTF( FontData::FontAwesome6_compressed_data, FontData::FontAwesome6_compressed_size, Round< float >( 13 * final_scale ), &merge_config, icon_glyph_range );
			}
			...
			if( FileSystem::FileExists( font_path ) )
			{
				...
				config.PixelSnapH = true;
				monospace_font = io.Fonts->AddFontFromFileTTF( WstringToString( font_path ).c_str(), Round< float >( 14 * final_scale ), &config );
			}
#endif
#endif
		}
		...
		HRESULT GUIResourceManager::OnCreateDevice(IDevice* pd3dDevice)
		{
			...
			NotifyGUIListeners( GUIInstance::DeviceCallbacks, &GUIInstance::OnCreateDevice, pd3dDevice );
#endif
			return S_OK;
		}
		...
		void GUIResourceManager::OnLostDevice()
		{
			...
			NotifyGUIListeners( GUIInstance::DeviceCallbacks, &GUIInstance::OnLostDevice );
#endif
		}

		void GUIResourceManager::OnResetDevice( IDevice* device, const Device::SurfaceDesc* backbuffer_surface_desc )
		{
#if DEBUG_GUI_ENABLED
			if (destroyed)
				return;

			NotifyGUIListeners( GUIInstance::DeviceCallbacks, &GUIInstance::OnResetDevice, device, backbuffer_surface_desc );
#endif
		}
		...
		HRESULT GUIResourceManager::OnDestroyDevice()
		{
			...
			NotifyGUIListeners( GUIInstance::DeviceCallbacks, &GUIInstance::OnDestroyDevice );
#endif
			return S_OK;
		}
