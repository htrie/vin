...
inline MaterialHandle DetermineMaterialOverride( const MaterialHandle& original, const Terrain::TileMaterialOverridesHandle& overrides1, const Terrain::TileMaterialOverridesHandle& overrides2 )
{
	if( overrides1 )
	{
		MaterialHandle mat = overrides1->GetMaterial( original );
		if( mat != original )
			return mat;
	}
	if( overrides2 )
...
{
public:
	MaterialHandle GetSegmentMaterial( const MeshHandleType& mesh, const unsigned segment_index ) const
	{
	...
	std::vector< MaterialHandle > GetAllMaterials( const MeshHandleType& mesh, const Terrain::TileMaterialOverridesHandle& fallback_overrides ) const
	{
	...
	MaterialHandle GetMaterial( const MeshHandleType& mesh, const unsigned segment_index, const Terrain::TileMaterialOverridesHandle& fallback_overrides ) const
	{
		const auto found = material_infos.find( mesh );
		if (found != material_infos.end())
		{
	...
	void AddMaterialOverrideInfo( const MeshHandleType& mesh, const unsigned segment_index, MaterialHandle material )
	{
...
protected:
	...
	typedef Memory::SmallVector< Info, 16, Memory::Tag::Mesh > InfoList_t;
	...
	void LoadMaterialOverride( const MeshHandleType& mesh, const std::wstring& segment_name, const std::wstring& material )
	{
		...
		if( segment_index < mesh->GetNumSegments() )
			AddMaterialOverrideInfo( mesh, segment_index, MaterialHandle( material ) );
#if defined(DEVELOPMENT_CONFIGURATION) || defined(TESTING_CONFIGURATION)
		else
		...
		if (found != material_infos.end())
		{
			for (const auto& [index, info] : enumerate( found->second ) )
			{
				if (info.material != mesh->GetMaterial(info.segment_index))
				{
					...
					stream << L"\"" << info.material.GetFilename().c_str() << L"\"";
