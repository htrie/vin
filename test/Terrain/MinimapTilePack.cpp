...
//#define LOG_TILE_MEMORY

namespace Terrain
{
	namespace
	{
#ifdef LOG_TILE_MEMORY
		size_t total_data_size = 0;

		size_t GetDataSize( const std::wstring_view& str )
		{
			return sizeof( std::wstring_view::value_type ) * ( str.length() + 1 );
		}

		template< typename K, typename V >
		size_t GetDataSize( const Memory::FlatMap< std::wstring, V, Memory::Tag::Game >& um )
		{
			size_t total = 0;
			for( size_t i = 0; i < um.bucket_count(); ++i )
			{
				const size_t bucket_size = um.bucket_size( i );
				total += sizeof( typename Memory::FlatMap< K, V, Memory::Tag::Game >::value_type ) * std::max< size_t >( 1, bucket_size );
			}
			return total;
		}
#endif
	...
	void MinimapTilePack::Save( const std::wstring& filename ) const
	{
		...
		if( render_target )
		{
			try
			{
				...
				auto compressed_texture = Device::Texture::CreateTexture("Minimap Tile Pack Compressed", device, width, height, 1, Device::UsageHint::DEFAULT, Device::PixelFormat::DXT5, Device::Pool::SYSTEMMEM, true, false, false);

				compressed_texture->CopyResource(texture.Get());
...
#ifdef LOG_TILE_MEMORY
			data_size += GetDataSize( tile_filename );
#endif
...
#ifdef LOG_TILE_MEMORY
		//data_size += sizeof(this) + GetDataSize( texture_filename ) + GetDataSize( folder_name ) + (texture ? sizeof(*texture) : 0) + GetDataSize( tile_rects );
		total_data_size += data_size;
		LOG_DEBUG( L"Loaded minimap file " << filename << L". Data size=" << data_size << L" Total data size=" << total_data_size );
#endif
	}
