...
namespace Terrain
{
	...
	void GridVisualisation::SetTransform( const simd::matrix& world )
	{
		...
		if (plane_entity_id)
		{
			auto plane_uniforms = Entity::Temp<Renderer::DrawCalls::Uniforms>(plane_entity_id);
			plane_uniforms->Add(Renderer::DrawDataNames::WorldTransform, world_transform);
			Entity::System::Get().Move(plane_entity_id, bounding_box, true, std::move(plane_uniforms));
		}

		if (line_entity_id)
		{
			auto line_uniforms = Entity::Temp<Renderer::DrawCalls::Uniforms>(line_entity_id);
			line_uniforms->Add(Renderer::DrawDataNames::WorldTransform, world_transform);
			Entity::System::Get().Move(line_entity_id, bounding_box, true, std::move(line_uniforms));
		}
	}
	...
	void GridVisualisation::SetupSceneObjects(uint8_t scene_layers)
	{
		...
		auto plane_desc = Entity::Temp<Entity::Desc>(plane_entity_id);
		plane_desc->SetVertexElements(vertex_elements)
			...
			.AddObjectUniforms(Renderer::DrawCalls::Uniforms().Add(Renderer::DrawDataNames::WorldTransform, world_transform))
			.AddObjectUniforms(Renderer::DrawCalls::Uniforms().Add(Renderer::DrawDataNames::FlipTangent, 1.0f));
		Entity::System::Get().Create(plane_entity_id, std::move(plane_desc));

		line_entity_id = Entity::System::Get().GetNextEntityID();
		auto line_desc = Entity::Temp<Entity::Desc>(line_entity_id);
		line_desc->SetVertexElements(vertex_elements)
			...
			.AddObjectUniforms(Renderer::DrawCalls::Uniforms().Add(Renderer::DrawDataNames::WorldTransform, world_transform))
			.AddObjectUniforms(Renderer::DrawCalls::Uniforms().Add(Renderer::DrawDataNames::FlipTangent, 1.0f));
		Entity::System::Get().Create(line_entity_id, std::move(line_desc));
	}
