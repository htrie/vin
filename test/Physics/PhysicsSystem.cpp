...
namespace Physics
{
	...
	void RenderCollision(Device::Line & line, const simd::matrix & final_transform, Vector3f point, Vector3f normal, Vector3f particle_point)
	{
	...
	void RenderEllipsoid(Device::Line &line, const simd::matrix &view_projection, Coords3f coords, Vector3f radii)
	{
		simd::vector3 line_verts[2];
		line_verts[0] = MakeDXVector(coords.pos - coords.xVector * radii.x);
		line_verts[1] = MakeDXVector(coords.pos + coords.xVector * radii.x);
		line.DrawTransform(&line_verts[0], 2, &view_projection, 0xff333333);
		...
		for (float ang = 0; ang < 2.0f * pi; ang += step)
		{
			...
			line.DrawTransform(&line_verts[0], 2, &view_projection, 0xffff0000);
			...
			line_verts[1] = MakeDXVector(coords.pos
				...
				+ coords.zVector * radii.z * next_sin_ang);
			line.DrawTransform(&line_verts[0], 2, &view_projection, 0xff00ff00);
			...
			line.DrawTransform(&line_verts[0], 2, &view_projection, 0xff0000ff);
		}
	}

	void RenderCapsule(Device::Line &line, const simd::matrix &view_projection, Coords3f coords, float length, float radius0, float radius1)
	{
	...
	void RenderBox(Device::Line &line, const simd::matrix &view_projection, Coords3f coords, Vector3f half_size)
	{
		simd::vector3 line_verts[2];
...
//
	...
	void System::RenderWireframe(Device::Line & line, const simd::matrix & view_projection)
	{
		...
		{
			if(deformable_meshes.IsIndexAlive(deformable_mesh_index))
				deformable_meshes.GetByIndex(deformable_mesh_index)->RenderWireframe(line, view_projection);
		}
		...
		for (size_t ellipsoid_index = 0; ellipsoid_index < particle_system->GetEllipsoidObstaclesCount(); ellipsoid_index++)
		{
			...
			RenderEllipsoid(line, view_projection, ellipsoid.Get()->GetCoords(), ellipsoid.Get()->GetRadii());
		}
		...
		for (size_t sphere_index = 0; sphere_index < particle_system->GetSphereObstaclesCount(); sphere_index++)
		{
			...
			RenderEllipsoid(line, view_projection, sphere.Get()->GetCoords(), Vector3f(radius, radius, radius));
		}
		...
		for (size_t capsule_index = 0; capsule_index < particle_system->GetCapsuleObstaclesCount(); capsule_index++)
		{
			...
			RenderCapsule(line, view_projection, capsule_obstacle.Get()->GetCoords(), length, sphere0.Get()->GetRadius(), sphere1.Get()->GetRadius());
		...
		for (size_t box_index = 0; box_index < particle_system->GetBoxObstaclesCount(); box_index++)
		{
			...
			RenderBox(line, view_projection, box.Get()->GetCoords(), box.Get()->GetHalfSize());
		}
		...
		for(size_t particle_group_index = 0; particle_group_index < GetParticleSystem()->GetParticleGroupsCount(); particle_group_index++)
		{
			...
			for(size_t collision_index = 0; collision_index < particle_group_ptr->GetCollisionsCount(); collision_index++)
			{
				...
				RenderCollision(line, final_transform, collision_point, collision_normal, collision_particle_point);
			}
		}
		wind_system->RenderWireframe(line, final_transform);
		/*for (size_t particleIndex = 0; particleIndex < particle_system->GetParticlesCount(); particle_system++)
		{
			...
			for (float ang = 0; ang < 2.0f * pi; ang += step)
			{
				...
				line.DrawTransform(&line_verts[0], 2, &final_transform, 0xffff0000);
				...
				line_verts[1] = MakeD3DXVector(ellipsoidCoords.pos
					...
					+ ellipsoidCoords.zVector * ellipsoidSize.z * next_sin_ang);
				line.DrawTransform(&line_verts[0], 2, &final_transform, 0xff00ff00);
				...
				line.DrawTransform(&line_verts[0], 2, &final_transform, 0xff0000ff);
			}
		}*/
	}
