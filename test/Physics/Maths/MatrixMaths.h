...
bool MatrixInverse(Scalar *matrix, Scalar *invMatrix, const IndexType dimsCount, const Scalar tolerance = Scalar(1e-9))
{
  ...
  return GaussSolveUnsafe<Scalar, IndexType>(tmpMatrix, invMatrix, 0, 0, dimsCount, tolerance);
  delete [] tmpMatrix;
}
...
{
  for(IndexType i = 0; i < matrix0Height; i++)
  {
    for(IndexType j = 0; j < matrix1Width; j++)
    {
      Scalar sum = 0;
      for(IndexType k = 0; k < matrix0Width; k++)
      {
        sum += matrix0[matrix0Width * i + k] * matrix1[k * matrix1Width + j];
      }
      resMatrix[i * matrix1Width + j] = sum;
    }
  }
}
template<typename Scalar, typename IndexType>
