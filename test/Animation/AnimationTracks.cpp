...
namespace Animation
{
	...
	struct AnimationSample
	{
		...
		struct Desc
		{
			...
			Desc(unsigned int transform_count4, size_t num_auxiliary, bool is_masked) : transform_count4(transform_count4), num_auxiliary(num_auxiliary), is_masked(is_masked) {}
		};
		...
		AnimationSample(Desc desc)
		{
			...
			const auto weights_size_aligned = simd::align(sizeof(float) * desc.transform_count4 * 4, alignment);
			const auto bone_mask_size_aligned = desc.is_masked 
													? simd::align(sizeof(uint32_t) * desc.transform_count4 * 4, alignment)
													: (size_t)0;
		...
		void SetupBoneMask(const std::vector<unsigned>* mask_bone_indices)
		{
			for (auto bone_index : *mask_bone_indices)
			{
				bone_masks[bone_index] = ~(uint32_t)0;
			}
		}
	};
	...
	)
	{
		...
		tracks->instances.emplace_back(AnimationSample::Desc(tracks->transform_count4, tracks->num_auxiliary, mask_bone_indices != nullptr));
		...
		if (mask_bone_indices)
		{
			...
			instance.animation_sample.SetupBoneMask(mask_bone_indices);
		}
	...
	{
		TracksAddAnimationInternal(tracks, animation_set, layer, mask_bone_indices, animation_speed_multiplier, blend_time, begin, end, elapsed, playback_mode, AnimationInstance::Type::Main, weight );
	}
	...
	{
		TracksAddAnimationInternal( tracks, animation_set, layer, mask_bone_indices, animation_speed_multiplier, blend_time, begin, end, elapsed, playback_mode, AnimationInstance::Type::Main, 1.0f - blend_ratio );
	}
	...
	{
		TracksAddAnimationInternal( tracks, animation_set, layer, mask_bone_indices, animation_speed_multiplier, blend_time, begin, end, elapsed, playback_mode, AnimationInstance::Type::Secondary, blend_ratio );
	}
	...
	{
		TracksAddAnimationInternal(tracks, animation_set, layer, mask_bone_indices, animation_speed_multiplier, blend_time, begin, end, elapsed, playback_mode, AnimationInstance::Type::Additional, 1.0f);
	}
