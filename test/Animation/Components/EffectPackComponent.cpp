...
namespace Animation
{
	namespace Components
	{
		...
		void EffectPackStatic::SaveComponent( std::wostream& stream) const
		{
			...
			if( animation_controller_index != -1 )
			{
				...
				for( const std::unique_ptr< EffectPackEvent >& effect_event : effect_pack_events )
				{
					...
					if( effect_event->permanent )
					{
						PushIndent();
						WriteValueNamed( L"event_is_permanent", true );
					}
				}
			}
		...
		bool EffectPackStatic::SetValue( const std::string& name, const std::wstring& value )
		{
			...
			const auto ParseEffectPack = [&]() -> std::pair< unsigned, Trails::TrailsEffectHandle >
			{
				...
				//Find the bone group index from the name
				const auto* bone_groups = parent_type.GetStaticComponent< BoneGroupsStatic >( bone_groups_index );

				unsigned bone_group_index = bone_groups->BoneGroupIndexByName( WstringToString( wide_bone_group ) );
				if( bone_group_index == -1 )
					throw Resource::Exception() << L"No bone group with name " << wide_bone_group;

				return std::make_pair( bone_group_index, Trails::TrailsEffectHandle( filename ) );
			};
		...
		bool EffectPackStatic::SetValue( const std::string & name, const bool value )
		{
			...
			else if( name == "event_is_permanent" )
			{
				...
				effect_pack_events.back()->permanent = value;
				return true;
			}
			return false;
		}
		...
		void EffectPack::AddEffectPack( const Visual::EffectPackHandle& effect_pack, const bool ignore_bad, const bool filter_submesh, const float remove_after_duration, const simd::matrix& attached_transform, const bool remove_on_animation_end )
		{
			...
			assert( !remove_on_animation_end || remove_after_duration == 0.0f );  // Can cause issues with reference counting if both parameters are set
			...
			if( existing != effect_packs.end() )
			{
				...
				{
					if( existing->remove_after_duration > 0.0f )
					{
						existing->remove_after_duration = std::max( existing->remove_after_duration, remove_after_duration );
					}
					else
					{
						++existing->ref_count;
						existing->remove_after_duration = remove_after_duration;
					}
				}
		...
		void EffectPack::AddEffectPackVisuals( RefCountedEffect& effect, const bool ignore_bad_final )
		{
			...
			if( !effect.filter_submesh )
			{
				...
				for( const auto& attached : attached_objects_ex )
				{
					...
					const auto scale_precision = 1000.f;
					const auto min_scale = ( unsigned )( std::max( attached.min_scale, 1.f / scale_precision ) * scale_precision );
					const auto max_scale = ( unsigned )( std::max( attached.max_scale, attached.min_scale ) * scale_precision );
					const auto random_scale = generate_random_number( min_scale, max_scale ) / scale_precision;
					const auto random_rotation = generate_random_number( ( unsigned )attached.min_rotation, ( unsigned )attached.max_rotation ) * PI / 180.f;
		...
		void EffectPack::OnAnimationEvent( const ClientAnimationController& controller, const Event& e, float time_until_trigger)
		{
			if( e.event_type == EffectPackEventType )
			{
				...
				const float duration = effect_event.permanent ? 0.0f : effect_event.length / controller.GetCurrentAnimationSpeed();
				const bool remove_on_animation_end = effect_event.length <= 0.0f && !effect_event.permanent;
				AddEffectPack( effect_event.effect_pack, false, false, duration, simd::matrix::identity(), remove_on_animation_end );
			}
		}
		...
		void EffectPack::RemoveEffectPacksOnAnimationEnd()
		{
			for( size_t i = effect_packs.size(); i-- > 0; )
			{
				...
				if( epk.remove_on_animation_end )
				{
