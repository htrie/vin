...
namespace Animation
{
	...
	namespace Components
	{
		...
		void ClientAnimationController::AdvanceAnimations( const float elapsed_time )
		{
			...
			if (compute_detailed_animation && hasMesh && !physics_driven_bone_infos.empty()) // Using physics to determine bone positions
				compute_detailed_animation = false; 

			// We have morphing bones
			if(compute_detailed_animation && morphing_info.dst_object.lock())
				compute_detailed_animation = false;
		...
		void ClientAnimationController::OnAnimationStart( const AnimationController&, const CurrentAnimation& new_animation, const bool blend )
		{
		...
		void ClientAnimationController::OnAppendAnimation(const AnimationController& controller, const CurrentAnimation& animation, const bool blend, const std::string& mask_bone_group)
		{
			const auto* animation_controller = GetObject().GetComponent< AnimationController >(static_ref.animation_controller_index);
