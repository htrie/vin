...
namespace Animation
{
	namespace Components
	{
		...
		struct EffectsDrawCallGraphInfo
		{
			EffectsDrawCallGraphInfo(const std::shared_ptr<Renderer::DrawCalls::InstanceDesc>& instance, const void *submesh_source, const bool parent_only, const bool base_graph)
				: instance(instance), submesh_source(submesh_source), parent_only(parent_only), base_graph(base_graph)
			{}
			std::shared_ptr<Renderer::DrawCalls::InstanceDesc> instance;
			const void* submesh_source = nullptr;
			bool parent_only = false;
			bool base_graph = false;
		};

		struct EffectsDrawCallMaterialInfo
		{
			EffectsDrawCallMaterialInfo() noexcept {}
			EffectsDrawCallMaterialInfo(const MaterialHandle& material, Visual::ApplyMode apply_mode, const void *submesh_source, const bool parent_only)
		...
		struct EffectsDrawCalls
		{
			...
			typedef Memory::Vector<EffectsDrawCallGraphInfo, Memory::Tag::EffectGraphInstances> EffectsDrawCallGraphInfos;
			EffectsDrawCallGraphInfos effect_graphs; // Note: these should basically be outline related graphs now
		...
		public:
			...
			void AddMaterialPass(MaterialHandle material, const Visual::ApplyMode apply_mode, const uint64_t source, const void* submesh_source = nullptr, const bool parent_only = false);
			void RemoveMaterialPass(MaterialHandle material, const uint64_t source);
			...
			std::shared_ptr<Renderer::DrawCalls::InstanceDesc> outline_colour_graph;
			std::shared_ptr<Renderer::DrawCalls::InstanceDesc> outline_colour_zprepass_graph;
			std::shared_ptr<Renderer::DrawCalls::InstanceDesc> outline_stencil_graph;
		...
		private:
			...
			void AddEffectGraphs(const Renderer::DrawCalls::InstanceDescs& instances, uint64_t source, uint64_t wait_on_source, const void* submesh_source, const bool parent_only, const bool base_graph);
			void RemoveEffectGraphs(const Renderer::DrawCalls::InstanceDescs& instances, uint64_t source);
