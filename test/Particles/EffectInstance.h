...
namespace Particles
{
	...
	private:
		...
		GpuParticles::Emitter CreateGpuEmitter(size_t i, float delay, float emitter_duration);
		GpuParticles::Emitter CreateCpuEmitter(size_t i, float delay, float emitter_duration);
		...
		struct EmitterInfo
		{
			...
			EmitterInfo() = default;
			EmitterInfo(size_t index, const GpuParticles::Emitter& id, bool converted) : index(index), emitter(id), converted(converted) {}
		};
