...
namespace Device
{
	...
	std::unique_ptr<Font> Font::Create(IDevice* device, INT Height, UINT Width, UINT Weight, UINT MipLevels, BOOL Italic, DWORD CharSet, DWORD OutputPrecision, DWORD Quality, DWORD PitchAndFamily, LPCWSTR pFaceName)
	{
		PROFILE_ONLY(Memory::StackTag tag(Memory::Tag::Device);)
		return std::make_unique<Font>(device, Height, Width, Weight, MipLevels, Italic, CharSet, OutputPrecision, Quality, PitchAndFamily, pFaceName);
	}
	...
	Font::Font(IDevice* device, int Height, UINT Width, UINT Weight, UINT MipLevels, BOOL Italic, DWORD CharSet, DWORD OutputPrecision, DWORD Quality, DWORD PitchAndFamily, LPCWSTR pFaceName)
		: internal_line(Line::Create(device, "Font Line")), font_height((float)Height)
	{
	}
	...
	void Font::DrawText(SpriteBatch* sprite_batch, const std::string& text, Rect& rect, DWORD format, const simd::color& color, float user_scale)
	{
		const bool use_internal_line = sprite_batch == nullptr;
		auto& line = use_internal_line ? internal_line : static_cast<SpriteBatch*>(sprite_batch)->line;

		line->SetWidth(1.0f);

		if (use_internal_line)
			line->Begin();

		const unsigned length = (unsigned)text.length();
		const float scale = user_scale * font_height / 10.0f;
	...
	void Font::ReleaseDC(HDC hDC)
	{
	}

	void Font::OnLostDevice()
	{
		internal_line->OnLostDevice();
	}

	void Font::OnResetDevice()
	{
		internal_line->OnResetDevice();
	}

	void Font::DrawTextW( SpriteBatch* pSprite, LPCWSTR pString, int count, Rect* pRect, DWORD Format, const simd::color& Colour, float scale)
	{
		DrawText(pSprite, WstringToString(pString), *pRect, Format, Colour, scale);
	}

	void Font::DrawTextA( SpriteBatch* pSprite, LPCSTR pString, int count, Rect* pRect, DWORD Format, const simd::color& Colour, float scale)
	{
		DrawText(pSprite, pString, *pRect, Format, Colour, scale);
	}
