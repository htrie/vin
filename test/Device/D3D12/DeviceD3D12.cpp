...
namespace Device
{
	...
	namespace
	{
		constexpr size_t CONSTANT_BUFFER_MAX_SIZE_D3D12 = 2 * Memory::MB;
		...
		, vsync(renderer_settings.vsync)
	{
		...
		auto resolution = GetSwapchainResolutionD3D12(device_settings, width, height, fullscreen, renderer_settings.window_mode == Renderer::Settings::WindowMode::BorderlessFullscreen);
		width = resolution.x;
		height = resolution.y;
	...
	SwapChain* IDeviceD3D12::GetMainSwapChain() const
	{
		return swap_chain.get();
	}

	HRESULT IDeviceD3D12::GetBackBufferDesc( UINT iSwapChain, UINT iBackBuffer, SurfaceDesc* pBBufferSurfaceDesc )
	{
		DXGI_SWAP_CHAIN_DESC swapChainDesc;
		HRESULT hr = ((SwapChainD3D12*)swap_chain.get())->GetSwapChain()->GetDesc(&swapChainDesc);
		if (SUCCEEDED(hr))
		{
