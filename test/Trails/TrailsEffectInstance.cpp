...
namespace Trails
{
		...
		const Renderer::DrawCalls::Bindings& static_bindings, const Renderer::DrawCalls::Uniforms& static_uniforms) // IMPORTANT: Keep in sync with TrailsTemplate::Warmup. // [TODO] Share.
	{
		...
		for (const auto& t : trails)
		{
			...
			desc->SetType(Renderer::DrawCalls::Type::Trail)
				...
				.AddObjectUniforms(Renderer::DrawCalls::Uniforms()
					.Add(Renderer::DrawDataNames::WorldTransform, simd::matrix::identity())
					.Add(Renderer::DrawDataNames::FlipTangent, 1.0f)
					.Add(Renderer::DrawDataNames::StartTime, -100.0f)
					.Add(Renderer::DrawDataNames::TrailTessellationU, t->trail_template->GetTessellationU())
					.Add(Renderer::DrawDataNames::TrailTessellationV, t->trail_template->GetTessellationV())
					.Add(Renderer::DrawDataNames::TrailDoubleSides, t->trail_template->double_trail)
					.Add(Renderer::DrawDataNames::TrailSplineU, t->trail_template->spline_u)
					.Add(Renderer::DrawDataNames::TrailSplineV, t->trail_template->spline_v)
					.Add(Renderer::DrawDataNames::TrailEmitterTransform, emitter_transform))
	...
	void TrailsEffectInstance::SetBoundingBox(const BoundingBox& bounding_box, bool visible)
	{
		for (const auto& entity : entities)
		{
			...
			uniforms->Add(Renderer::DrawDataNames::TrailEmitterTransform, emitter_transform);
			...
			uniforms->Add(Renderer::DrawDataNames::TrailTessellationU, entity.trail->trail_template->GetTessellationU());
			uniforms->Add(Renderer::DrawDataNames::TrailTessellationV, entity.trail->trail_template->GetTessellationV());
			uniforms->Add(Renderer::DrawDataNames::TrailSplineU, entity.trail->trail_template->spline_u);
			uniforms->Add(Renderer::DrawDataNames::TrailSplineV, entity.trail->trail_template->spline_v);
