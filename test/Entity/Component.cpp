...
namespace Entity
{
...

		{
...

			{
				...
				for (auto& uniform_input : inputs.uniform_inputs_infos)
				{
					...
					local_pipeline_uniforms.Add(uniform_key, std::move(uniform)); // Copy into local storage.
				}
	...
	Cull::Cull(const Desc& desc, Id render_id)
	...
	Render::Render(const Desc& desc, const std::shared_ptr<Template>& templ)
		: draw_call(Renderer::System::Get().CreateDrawCall(desc))
		, templ(templ)
	{
	}

	Particle::Particle(const Desc& desc, Id cull_id)
		: emitter(desc.emitter)
		, cull_id(cull_id)
	{
	}

	Trail::Trail(const Desc& desc, Id cull_id)
		: trail(desc.trail)
		, cull_id(cull_id)
	{
	}
