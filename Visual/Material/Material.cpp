...
// TODO: remove this constructor once all code is changed to Material System and have removed all instances of MaterialHandle
...
Material::Material(std::wstring_view filename)
	: id(Objects::Type::GetTypeNameHash(filename.data()))
	, filename(filename)
{
	PROFILE;
...
Material::Material(const MaterialCreator& creator)
	: filename(creator.filename), id(Objects::Type::GetTypeNameHash(creator.filename.data()))
{
	// TODO: conversion code, remove once conversion is done
	PROFILE;
...
std::tuple<Renderer::DrawCalls::Bindings, Renderer::DrawCalls::Uniforms, Renderer::DrawCalls::Bindings, Renderer::DrawCalls::Uniforms> Material::FindBaseTextures(const Material& material)
{
	...
	mat_override_uniforms.AddStatic(Renderer::DrawDataNames::OverrideAlbedoOpaque, Renderer::DrawCalls::Uniform(material.GetBlendMode() == Renderer::DrawCalls::BlendMode::Opaque));
	mat_override_uniforms.AddStatic(Renderer::DrawDataNames::OverrideNormalSwizzle, Renderer::DrawCalls::Uniform(base_normal_texture.name_id == 1));

	Renderer::DrawCalls::Uniforms tile_ground_uniforms;
	tile_ground_uniforms.AddStatic(Renderer::DrawDataNames::TileGroundNormalSwizzle, Renderer::DrawCalls::Uniform(base_normal_texture.name_id == 1));
...
{
	return MaterialDesc(handle->GetFilename());
}

MaterialHandle ConvertMaterialDesc(const MaterialDesc& desc)
{
	return MaterialHandle(desc.GetFilename());
}