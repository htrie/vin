...
namespace Device
{
	namespace Internal
	{
		...
		public:
			...
			void Draw(const simd::vector2* pVertexList, DWORD dwVertexListCount, const simd::color& Color, float thickness);
			void Draw(const simd::vector2* pVertexList, DWORD dwVertexListCount, const simd::color& StartColor, const simd::color& EndColor, float thickness);
			void DrawTransform(const simd::vector3* pVertexList, DWORD dwVertexListCount, const simd::matrix* pTransform, const simd::color& Color, float thickness);
		};
		...
		LineImpl::LineImpl( IDevice* device)
			: device(device)
		...
		void LineImpl::FlushQuad(Device::CommandBuffer& command_buffer, Device::Pass* pass, QuadBuffer& buffer)
		{
			if (const auto quad_draw_count = buffer.GetUsedCount())
			{
				...
				{
					return Device::Pipeline::Create("DebugQuad", device, pass, PrimitiveType::TRIANGLELIST, &vertex_declaration, vertex_shader.Get(), pixel_shader.Get(), DisableBlendState(), 
		...
		void LineImpl::FlushLine(Device::CommandBuffer& command_buffer, Device::Pass* pass, LineBuffer& buffer)
		{
			if (const auto line_draw_count = buffer.GetUsedCount())
			{
				...
				{
					return Device::Pipeline::Create("DebugLine", device, pass, PrimitiveType::LINELIST, &vertex_declaration, vertex_shader.Get(), pixel_shader.Get(), DisableBlendState(), 
						Device::UIRasterizerState().SetRenderMode(Device::CullMode::NONE, Device::FillMode::SOLID), Device::UIDepthStencilState());
				}).Get();
				if (command_buffer.BindPipeline(pipeline))
				{
		...
		void LineImpl::Begin()
		{
			dimensions = simd::vector2((float)device->GetBackBufferWidth(), (float)device->GetBackBufferHeight());
		}
		...
		void LineImpl::Draw(const simd::vector2* pVertexList, DWORD dwVertexListCount, const simd::color& Color, float thickness)
		{
			Draw(pVertexList, dwVertexListCount, Color, Color, thickness);
		}

		void LineImpl::Draw(const simd::vector2 *pVertexList, DWORD dwVertexListCount, const simd::color& StartColor, const simd::color& EndColor, float thickness)
		{
		...
		void LineImpl::DrawTransform(const simd::vector3 *pVertexList, DWORD dwVertexListCount, const simd::matrix* pTransform, const simd::color& color, float thickness)
		{
			if (dwVertexListCount < 2)
				return;
	...
	std::unique_ptr<Line> Line::Create(IDevice* device)
	{
		PROFILE_ONLY(Memory::StackTag tag(Memory::Tag::Device);)
		return std::make_unique<Line>(device);
	}

	Line::Line(IDevice* device)
		: impl(std::make_unique<Internal::LineImpl>(device))
	{
	}
	...
	void Line::Draw(const simd::vector2* pVertexList, DWORD dwVertexListCount, const simd::color& Color, float thickness)
	{
		impl->Draw(pVertexList, dwVertexListCount, Color, thickness);
	}

	void Line::DrawTransform(const simd::vector3* pVertexList, DWORD dwVertexListCount, const simd::matrix* pTransform, const simd::color& Color, float thickness)
	{
		impl->DrawTransform(pVertexList, dwVertexListCount, pTransform, Color, thickness);
	}
}
