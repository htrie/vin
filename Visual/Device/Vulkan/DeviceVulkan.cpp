...
namespace Device
{
	...
	{
	#define SILENT_MESSAGE do { static bool silence = false; if(silence) return VK_FALSE; silence = true; } while(false)

		if (pCallbackData->messageIdNumber == 0x3d492883) // The SPIR-V Extension was declared, but none of the requirements were met to use it
		{
			std::string_view msg = pCallbackData->pMessage;
			if (msg.find("SPV_GOOGLE_hlsl_functionality1") != msg.npos)
				SILENT_MESSAGE;
			else if (msg.find("SPV_GOOGLE_user_type") != msg.npos)
				SILENT_MESSAGE;
		}
		else if (pCallbackData->messageIdNumber == 0x609a13b) // Fragment shader writes to output with no matching attachment
			SILENT_MESSAGE;
		...
		// Throw so the calling site catch it, print some additional infos and safely proceed (like pipeline creation for instance). 
		if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
			throw vk::LogicError("Validation error");

		return VK_FALSE;

	#undef SILENT_MESSAGE
	}
	...
	void IDeviceVulkan::InitSwapChain()
	{
		...
		if (swap_chain)
			swap_chain->Recreate(window->GetWnd(), this, device_settings.BackBufferWidth, device_settings.BackBufferHeight, fullscreen, vsync, 0);
		else
			swap_chain = SwapChain::Create(window->GetWnd(), this, device_settings.BackBufferWidth, device_settings.BackBufferHeight, fullscreen, vsync, 0);
	...
	void IDeviceVulkan::ReinitSwapChain()
	{
		...
		if (auto* window = WindowDX::GetWindow())
		{
			...
			window->TriggerDeviceReset(this); // TODO: Avoid recreating everything, only need to resize main render target.
		}
	}
