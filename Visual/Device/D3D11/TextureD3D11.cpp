...
namespace Device
{
	...
	bool IsSet(UsageHint usage, UsageHint flag)
	{
		return !!(unsigned(usage) & unsigned(flag));
	}

	D3D11_USAGE GetTextureUsageD3D11(UsageHint usage, DXGI_FORMAT format, bool enable_gpu_write)
	{
		if (IsSet(usage, UsageHint::RENDERTARGET) || IsSet(usage, UsageHint::DEPTHSTENCIL) || IsSet(usage, UsageHint::DEFAULT) || format == DXGI_FORMAT_R24G8_TYPELESS) return D3D11_USAGE_DEFAULT;
		if (IsSet(usage, UsageHint::STAGING)) return D3D11_USAGE_STAGING;
		return D3D11_USAGE_DYNAMIC;
	}

	UINT GetBindFlagsD3D11(UsageHint usage, DXGI_FORMAT format, bool enable_gpu_write)
	{
		UINT bind_flags = 0;
		if (IsSet(usage, UsageHint::RENDERTARGET)) bind_flags |= D3D11_BIND_RENDER_TARGET;
		if (format == DXGI_FORMAT_R24G8_TYPELESS || IsSet(usage, UsageHint::DEPTHSTENCIL)) bind_flags |= D3D11_BIND_DEPTH_STENCIL;
		if (!IsSet(usage, UsageHint::STAGING)) bind_flags |= D3D11_BIND_SHADER_RESOURCE;
		return bind_flags;
	}

	UINT GetCpuAccessFlagsD3D11(UsageHint usage, DXGI_FORMAT format, bool enable_gpu_write)
	{
		if (IsSet(usage, UsageHint::STAGING)) return D3D11_CPU_ACCESS_READ | D3D11_CPU_ACCESS_WRITE;
		if (IsSet(usage, UsageHint::DYNAMIC)) return D3D11_CPU_ACCESS_WRITE;
		return 0;
	}
		...
		: TextureD3D11(name, device, useSRGB)
	{
		...
		desc.ArraySize = 6;
		desc.Format = ConvertPixelFormat(format, useSRGB);
		desc.Usage = GetTextureUsageD3D11(usage, desc.Format, enable_gpu_write);
		desc.BindFlags = GetBindFlagsD3D11(usage, desc.Format, enable_gpu_write);
		desc.CPUAccessFlags = GetCpuAccessFlagsD3D11(usage, desc.Format, enable_gpu_write);
		...
		//staging resources don't use resource views but the _mip_resource_views is still used to cache the number of mip levels
		{
			size_t mips_count = 0;
			ID3D11Texture2D* texture2d = nullptr;
			hr = _texture->QueryInterface(IID_GRAPHICS_PPV_ARGS(&texture2d));
		...
		{
			{
				...
				resourceviewdesc.TextureCube.MipLevels = desc.MipLevels == 0 ? UINT(-1) : desc.MipLevels;
			...
			{
				for (size_t layer = 0; layer < _mip_resource_views.size(); layer++)
				{
					D3D11_SHADER_RESOURCE_VIEW_DESC layer_resource_view;
					ZeroMemory(&layer_resource_view, sizeof(layer_resource_view));
					layer_resource_view.Format = (desc.Format == DXGI_FORMAT_R24G8_TYPELESS) ? DXGI_FORMAT_R24_UNORM_X8_TYPELESS : desc.Format; // NOTE: Needs typeless to avoid Debug Layer error.
					layer_resource_view.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2DARRAY;
					layer_resource_view.Texture2DArray.MipLevels = 1;
					layer_resource_view.Texture2DArray.ArraySize = 6;
					layer_resource_view.Texture2DArray.MostDetailedMip = (UINT)layer;

					ID3D11ShaderResourceView* pResourceView;
					hr = static_cast<IDeviceD3D11*>(device)->GetDevice()->CreateShaderResourceView(_texture.get(), &layer_resource_view, &pResourceView);
					if (FAILED(hr))
						throw ExceptionD3D11("CreateShaderResourceView", hr, static_cast<IDeviceD3D11*>(device)->GetDevice());
					_mip_resource_views[layer].reset(pResourceView);
				}
	...
	void TextureD3D11::CopySubResource(UINT level, UINT x, UINT y, UINT z, Texture* pSrcResource, UINT SrcSubResource, const Rect* pSrcRect)
	{
		...
		{
			hr = DirectX::CreateTexture(static_cast<IDeviceD3D11*>(pDevice)->GetDevice(), out, 1, out_scratch.GetMetadata(), &resource);
			if (FAILED(hr))
				throw ExceptionD3D11("CreateTexture", hr, static_cast<IDeviceD3D11*>(pDevice)->GetDevice());
		}
	...
	void TextureD3D11::TransferFrom(Texture* src_texture)
	{
		static_cast<IDeviceD3D11*>(pDevice)->GetContext()->CopyResource(_texture.get(), static_cast<TextureD3D11*>(src_texture)->_texture.get());
	}
	void TextureD3D11::TranscodeFrom(Texture* src_texture)
	{
		//this code works incorrectly for textures that have mips > 1 or array_size > 1
		CopySubResource(0, 0, 0, 0, src_texture, 0, nullptr);
	}
