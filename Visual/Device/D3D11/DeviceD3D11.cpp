...
namespace Device
{
	...
	#endif
		swap_chain = Device::SwapChain::Create(hFocusWindow, this, device_settings.BackBufferWidth, device_settings.BackBufferHeight, fullscreen, vsync, output);
	...
	bool IDeviceD3D11::CheckFullScreenFailed()
	{
		...
		return false;
	#else
		return ((SwapChainD3D11*)swap_chain.get())->GetFullscreenFailed();
	#endif
	}
	...
	HRESULT IDeviceD3D11::ResizeDXGIBuffers( UINT Width, UINT Height, BOOL bFullScreen, BOOL bBorderless /*=false*/ )
	{
		...
		// Resize buffers.
		hr = ((SwapChainD3D11*)swap_chain.get())->RecreateBuffers(Width, Height, Flags);
	#ifndef _XBOX_ONE
		if (FAILED(hr))
		{
	...
	{ 
		vsync = enabled; 
		if (swap_chain)
			((SwapChainD3D11*)swap_chain.get())->SetVSync(enabled);
	}
