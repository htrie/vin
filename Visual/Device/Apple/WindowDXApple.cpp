...
void Device::WindowDX::MainLoop()
{
...
#if defined(__APPLE__iphoneos)
	frame_time = (float)timer.GetElapsedTime();
#endif
	...
	Bridge::SetCursor(cursor_id); // Need to be called every frame.
#endif
...
{
#ifdef __APPLE__macosx
	return Bridging::GetKeyState( key );
#else
    return 0;
#endif
}
...
{
#if defined(__APPLE__macosx)
	return Bridge::IsActive(); // No way to get foreground on macOS, so we use active here as well.
#else
	return true;
#endif
}
...
{
#ifdef __APPLE__macosx
	Bridge::SetWindowResolution(hInstance, client_width, client_height);
#endif
...
{
#ifdef __APPLE__macosx
    return Bridge::Message(Msg, wParam, lParam);
#else
    return 0;
#endif
}
...
INT Device::WindowDX::MessageBox( LPCSTR text, LPCSTR caption, UINT )
{
    ...
    Bridge::MessageBox(text, caption, 0);
	return 0;
}
...
INT Device::WindowDX::MessageBox( LPCWSTR text, LPCWSTR caption, UINT )
{
    ...
    Bridge::MessageBox(WstringToString( text ), WstringToString( caption ), 0);
	return 0;
}
