...
namespace Terrain
{
	...
	void Minimap::OnResetDevice(Device::IDevice* device)
	{
		screen_width = device->GetBackBufferWidth();
		screen_height = device->GetBackBufferHeight();
	...
	std::shared_ptr< MinimapTilePack > Minimap::CreateTilePack( Device::IDevice* device )
	{
		...
		for( const Location& loc : bound )
		{
			...
			if( tile.IsNull() )
				continue;
			while( tile->UseInheritedMinimapTile() )
				tile = tile->GetInheritedTDT();
...
/*			size_t maxIndex = 0;
	...
	void Minimap::CreateTilemapMesh( Device::IDevice* device )
	{
		...
		const auto ReadMinimapTile = [&](Location loc) -> MinimapTileDesc
		{
			...
			if (!res_tile.tile.IsNull())
			{
				...
				if (res_tile.pack_find_result.first == nullptr)
				{
	...
	void Minimap::CreateWalkabilityMesh(Device::IDevice* device)
	{
		...
		const size_t width = size_t(minimap_size.x * TileSize / stride + 0.5f);
		const size_t height = size_t(minimap_size.y * TileSize / stride + 0.5f);
		...
		std::vector<unsigned int > indices;
		for( int diag = (int)width - 1 + (int)height - 1 - 1; diag >= 0 ; --diag )
		{
			int x = std::min( diag, (int)width - 1 - 1 );
			int y = std::max( 0, diag - (int)width + 1 + 1 );
			for( ; y < height - 1 && x >= 0; --x, ++y )
			{
				...
				{
					base_indices[quad_vertex] = base_locations[quad_vertex].x + base_locations[quad_vertex].y * (int)width;
				}
	...
	void Minimap::CreatePOIVertices( Device::IDevice* device )
	{
		...
		for( unsigned i = 0; i < Loaders::MinimapIconsValues::NumRows; ++i )
		{
			...
			constexpr auto subgraph_index = -1.f;
	...
	void Minimap::UploadTileVisibility()
	{
		...
		visibility_shader.pixel_uniform_buffer->SetFloat("visibility_radius", Clamp( 4.0f * visibility_radius_mult, 2.0f, 10.0f ) );
	...
	void Minimap::SetVisibilityMultiplier( const float mult )
	{
		visibility_radius_mult = mult;
	}
