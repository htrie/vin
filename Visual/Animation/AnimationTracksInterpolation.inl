...
template<typename F4, typename U4>
void UpdateWeightMaskedOverride( Tracks* tracks, Animation::AnimationInstance& instance )
{
	auto& weights = instance.animation_sample.weights;

	for( uint32_t i = 0; i < tracks->transform_count4 * 12; i += 4 )
	{
		F4 bone_mask = F4::select( ( U4& )( instance.animation_sample.bone_masks[i] ), F4( 1.0f ), F4( 0.0f ) );
		F4& weight = ( F4& )( weights[i] );
		weight = weight + F4( instance.weight ) * bone_mask;
...
template <typename F4x4, typename F4x3, typename F4>
std::tuple<F4x3, F4x3, F4x4> BlendMaskedOverride(const AnimationSample& animation_sample, unsigned int i, F4x3& scale, F4x3& translation, F4x4& rotation, const F4& elapsed, const unsigned num_weights)
{
	F4 scale_weight = (F4&)animation_sample.weights[i * 4];
	F4 translation_weight = (F4&)animation_sample.weights[i * 4 + num_weights];
	F4 rotation_weight = (F4&)animation_sample.weights[i * 4 + num_weights * 2];

	const F4x3 override_scale = TracksInterpolateScaleTime<F4x3, F4>(animation_sample, i, elapsed);
	const F4x3 override_translation = TracksInterpolateTranslationTime<F4x3, F4>(animation_sample, i, elapsed);
	const F4x4 override_rotation = TracksInterpolateRotationTime<F4x4, F4>(animation_sample, i, elapsed);

	return std::make_tuple(
		F4x3::lerp(scale, override_scale, scale_weight),
		F4x3::lerp(translation, override_translation, translation_weight),
		F4x4::slerp(rotation, override_rotation, rotation_weight)
	);
}
