...
#include "Visual/Renderer/MaterialSystem.h"
...
namespace Animation
{
	namespace Components
	{
		...
		PassIds GetPassIds(const Visual::MaterialPasses& intrinsic_material_passes, const Visual::EffectPackHandle& epk)
		{
			...
			const bool has_outline_colour_zprepass)
		{
			...
			auto final_mat_desc = default_material;
			for (const auto& material_pass : material_passes)
			{
				if (material_pass.apply_mode == Visual::ApplyMode::StandAlone)
					final_mat_desc = material_pass.material;
			}
			const auto final_material = Mat::System::Get().FindMat(final_mat_desc);
			for (const auto& desc : final_material->GetGraphDescs())
				out.graph_descs.push_back(desc);

			if (has_outline_stencil)
			{
				out.graph_descs.push_back(L"Metadata/EngineGraphs/outline_stencil.fxgraph");
			}
			else if (has_outline_colour)
			{
				out.graph_descs.push_back(L"Metadata/EngineGraphs/outline_colour.fxgraph");

				if (has_outline_colour_zprepass)
					out.graph_descs.push_back(L"Metadata/EngineGraphs/outline_colour_zprepass.fxgraph");
			}

			out.blend_mode = Renderer::DrawCalls::BlendMode::Opaque;
			for (auto& graph_desc : out.graph_descs)
				if (const auto graph = EffectGraph::System::Get().FindGraph(graph_desc.GetGraphFilename()); graph->IsBlendModeOverriden())
		...
		void GatherPassGraphs(GatherOut& out, const MaterialDesc& default_material, const EffectsDrawCalls::EffectsDrawCallMaterialInfos& material_passes, const void* drawcall_source)
		{
			const auto default_mat = Mat::System::Get().FindMat(default_material);
			auto has_constant = default_mat->HasConstant();
			for (const auto& material_pass : material_passes)
			{
				if (material_pass.apply_mode == Visual::ApplyMode::Append)
				{
					const auto mat = Mat::System::Get().FindMat(material_pass.material);
					for (const auto& desc : mat->GetGraphDescs())
		...
		{
			GatherOut result;
			GatherBaseGraphs(result, default_material, material_passes, has_uv_alpha, mesh_flags, has_outline_stencil, has_outline_colour, has_outline_colour_zprepass);
			GatherPassGraphs(result, default_material, material_passes, drawcall_source);
			return result;
		}
			...
			const uint32_t mesh_flags)
		{
			for (auto pass_id : pass_ids)
			{
				...
				const auto gather = GatherGraphs(default_material, material_passes, has_uv_alpha, nullptr, mesh_flags);
				...
				if (skin_mesh_component != -1)
				{
					...
					if (const auto* ref = GetParentType().GetStaticComponent<SkinMeshStatic>(skin_mesh_component))
					{
						for (auto& mesh : ref->meshes)
						{
							...
							{
								if (auto material = mesh.mesh->GetMaterial(i); material.GetHash() != 0)
		...
		AnimatedRender::AnimatedRender( const AnimatedRenderStatic& static_ref, Objects::Object& object )
			...
			, object_random_seed(simd::RandomUIntLocal())
			, synced_random_seed(simd::RandomUIntLocal())
		{
		...
		bool AnimatedRender::AreResourcesReady() const
		{
			if( static_ref.skin_mesh_component != -1 )
			{
				if (const SkinMesh* skin_mesh = GetObject().GetComponent< SkinMesh >( static_ref.skin_mesh_component ))
				{
					...
					{
						const auto& animated_mesh = mesh_data->mesh;
						const auto animated_mesh_raw_data = mesh_data->mesh->GetMesh();
			...
			const bool has_outline_colour_zprepass)
		{
			...
			const auto gather = GatherGraphs(material, edc.material_passes, has_uv_alpha, drawcall_source, mesh_flags, has_outline_stencil, has_outline_colour, has_outline_colour_zprepass);
		...
		void AnimatedRender::RecreatePassEntities(EffectsDrawCalls& edc, EffectsDrawCalls* wait_on_edc,
			...
			const bool has_outline_colour_zprepass)
		{
			const auto previous_entities = Copy( edc.entities );
			for (auto& entity : edc.entities)
				Entity::System::Get().Destroy(entity.entity_id);
			edc.entities.clear();

			const auto ForEachSkinnedMeshSegments = [&](std::function<void(const SkinMesh::AnimatedMeshData& mesh_data, const SkinMesh::AnimatedMeshData::Section& section)> func)
			{
				if (static_ref.skin_mesh_component != -1)
				{
			...
			const auto ForEachFixedMeshSegments = [&](std::function<void(const Mesh::FixedMeshHandle& mesh, const unsigned segment_index)> func)
			{
				if (static_ref.fixed_mesh_component != -1)
				{
					...
					for( size_t mesh_index = 0; mesh_index < fixed_meshes.size(); ++mesh_index )
					{
						if( !fixed_mesh_component->GetMeshVisibility()[ mesh_index ] )
							continue;

						const auto& segments = fixed_meshes[ mesh_index ].mesh->Segments();
						for( unsigned segment_index = 0; segment_index < segments.size(); ++segment_index )
						{
							if( fixed_mesh_component->GetSegmentVisibility( mesh_index )[ segment_index ] )
								func( fixed_meshes[ mesh_index ].mesh, segment_index );
				}
					}
				}
			};
			...
			if( static_ref.skin_mesh_component != -1 )
			{
				if (ClientAnimationController* animation_controller = GetController())
				{
					...
					{
						const auto material_handle = Mat::System::Get().FindMat(section.material);
						const auto entity_id = Entity::System::Get().GetNextEntityID();

						auto desc = Entity::Desc()
							.SetType(Renderer::DrawCalls::Type::Animate)
							...
							.AddObjectUniforms(Renderer::DrawCalls::Uniforms()
								...
								.AddStatic(Renderer::DrawDataNames::StartTime, edc.start_time)
								.AddStatic(Renderer::DrawDataNames::RandomSeed, object_random_seed)
								.AddDynamic(Renderer::DrawDataNames::OutlineColour, outline_color))
							...
							.AddObjectBindings(dynamic_bindings)
							.AddObjectUniforms(static_uniforms)
							.SetPalette(animation_controller->GetPalette())
							.SetDebugName(GetObject().GetType().GetFilename());

						Renderer::DynamicParameters dynamic_parameters;
						GatherAllEffectGraphs(desc, dynamic_parameters, edc, animation_controller, section.material, mesh_data.source, mesh_data.mesh->GetMesh()->GetFlags(),
							has_outline_stencil, has_outline_colour, has_outline_colour_zprepass);
			...
			if( static_ref.fixed_mesh_component != -1 )
			{
				...
				ForEachFixedMeshSegments( [ & ]( const Mesh::FixedMeshHandle& mesh, const unsigned segment_index )
				{
					...
					const auto& material_desc = fixed_mesh_component->GetMaterial(mesh, segment_index);
					const auto material_handle = Mat::System::Get().FindMat(material_desc);

					const auto entity_id = Entity::System::Get().GetNextEntityID();

					auto desc = Entity::Desc()
						.SetType(Renderer::DrawCalls::Type::Fixe)
						.SetAsync(!GetObject().IsHighPriority())
					...
					Renderer::DynamicParameters dynamic_parameters;
					GatherAllEffectGraphs(desc, dynamic_parameters, edc, nullptr, material_desc, nullptr, mesh->GetFlags(),
							has_outline_stencil, has_outline_colour, has_outline_colour_zprepass);
		...
		void AnimatedRender::SetupSceneObjects(uint8_t scene_layers, Decals::DecalManager* decal_manager )
		{
			...
			static_uniforms.AddStatic(Renderer::DrawDataNames::SyncedRandomSeed, synced_random_seed);
		...
		void AnimatedRender::EnableOutline( const simd::vector4& colour )
		{
			...
			for (unsigned a = 0; a < outline_color_pass; a++)
				AddOutlineEffectGraphs(a, (uint64_t)-1);

			// set the second pass id to another value so as to not conflict with any built-in second pass effects
			AddOutlineEffectGraphs(outline_color_pass, 0); // Wait-on first pass to avoid outline glitch (scaled-up shader ready before masked-out shader).
		}
		...
		void AnimatedRender::DisableOutline( )
		{
			...
			for (unsigned a = 0; a < outline_color_pass; a++)
				RemoveOutlineEffectGraphs(a);

			// second pass outline effect is using 2 as the pass id
			RemoveOutlineEffectGraphs(outline_color_pass);
		}
		...
		{
			simd::matrix transform = GetTransform();
		...
		void AnimatedRender::RenderFrameMoveInternal( const float elapsed_time )
		{
...

			{
				const auto push_entity = [&](auto& entity)
				{
					...
					Entity::System::Get().Move(entity.entity_id, current_bounding_box, !IsFlagSet(Flags::RenderingDisabled), uniforms);
				};
		...
		void AnimatedRender::AddOutlineEffectGraphs(uint64_t source, uint64_t wait_on_source)
		{
			EffectsDrawCalls &edc = getEffectsDrawCalls(source);

			if (scene_setup)
			{
				const bool has_outline_stencil = wait_on_source == (uint64_t)-1;
				const bool has_outline_colour = !has_outline_stencil;
				const bool has_outline_colour_zprepass = has_outline_colour && IsFlagSet(Flags::OutlineZPrepass);
				RecreatePassEntities(edc, wait_on_source != (uint64_t)-1 ? &getEffectsDrawCalls(wait_on_source) : nullptr, has_outline_stencil, has_outline_colour, has_outline_colour_zprepass);
			}

			if (static_ref.attached_object_component != -1)
			{
				const auto* attached_object_component = GetObject().GetComponent< AttachedAnimatedObject >(static_ref.attached_object_component);
				for (auto& object : attached_object_component->GetAttachedObjects())
				{
					if (object.effects_propagate)
					{
						auto* animated_render = object.object->GetComponent< AnimatedRender >();
						if ((source == 0) && !animated_render->static_ref.ignore_parent_firstpass)
							animated_render->AddOutlineEffectGraphs(source, wait_on_source);
		...
		void AnimatedRender::RemoveOutlineEffectGraphs(uint64_t source)
		{
			if (EffectsDrawCalls* edc = findEffectsDrawCalls(source))
			{
				if (source && edc->material_passes.empty())
				...
				if (static_ref.attached_object_component != -1)
				{
					...
					{
						if (object.effects_propagate)
							object.object->GetComponent< AnimatedRender >()->RemoveOutlineEffectGraphs(source);
					}
				}
			}
		}

		void AnimatedRender::AddMaterialPass(const MaterialDesc& material, const Visual::ApplyMode apply_mode, const uint64_t source, const void* submesh_source /*= nullptr*/, const bool parent_only /*= false*/)
		{
			EffectsDrawCalls &edc = getEffectsDrawCalls(source);
		...
		void AnimatedRender::RemoveMaterialPass(const MaterialDesc& material, const uint64_t source)
		{
			if (EffectsDrawCalls *edc = findEffectsDrawCalls(source))
			{
				auto found = std::find_if(edc->material_passes.begin(), edc->material_passes.end(), [&](const auto& material_override) { 
					return material_override.material.GetHash() == material.GetHash(); 
				});
