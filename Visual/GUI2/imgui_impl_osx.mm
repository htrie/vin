...
{
	g_view = (__bridge NSView*)window->GetInstance();
    ...
    [g_view addSubview:g_KeyEventResponder];
...
static void ImGui_Platform_UpdateMouseCursor()
{
    ImGuiIO& io = ImGui::GetIO();
...
static void ImGui_Platform_UpdateImePosWithView(NSView* view)
{
    ImGuiIO& io = ImGui::GetIO();
    if (io.WantTextInput)
...
void ImGui_Platform_NewFrame()
{
    ...
    if (g_view)
    {
        const float dpi = (float)[g_view.window backingScaleFactor];
    ...
    ImGui_Platform_UpdateMouseCursor();
    ImGui_Platform_UpdateImePosWithView(g_view);
}

void  ImGui_Platform_EnableDpiAwareness() {} // Not Implemented
float ImGui_Platform_GetDpiScaleForHwnd(void* hwnd) { return 1.0f; } // Not Implemented
float ImGui_Platform_GetDpiScaleForMonitor(void* monitor) { return 1.0f; } // Not Implemented
