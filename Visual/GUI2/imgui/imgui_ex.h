...
namespace ImGuiEx
{
    ...
    bool InputStringWithHint( const std::string_view, const std::string_view hint, std::string& str, ImGuiInputTextFlags flags = 0 );
    ...
    Finally BeginFocused( const char* name, const char** set_last_focused, bool* open = nullptr, ImGuiWindowFlags flags = 0 );
    ...
    inline bool PassFilter( const ImGuiTextFilter& filter, const std::string_view text ) { return filter.PassFilter( text.data(), text.data() + text.size() ); }
	...
	bool ColorEditSRGB(const std::string_view label, float linear_col[N], ImGuiColorEditFlags flags = 0)
	{
		...
		if( ColorEdit<N>(label, srgb_col, flags) )
        {
		    for (int i = 0; i < N; i++)
