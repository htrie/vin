...
namespace Renderer
{
	...
	FragmentLinker::Fragment& FragmentLinker::Fragment::operator=( Fragment&& other )
	{
		...
		inclusions = std::move( other.inclusions );
		has_side_effects = std::move(other.has_side_effects);
	...
	void FragmentLinker::AddFileNode(const std::wstring& file, Memory::Set<std::string, Memory::Tag::ShaderLinker>& node_uniforms)
	{
		...
		try {
			...
			// add the group node
			nodes[DrawCalls::GroupNodeName] = Fragment(); 
			nodes[DrawCalls::GroupNodeName].is_group = true;
		} catch (const Resource::Exception& e)
		{
			throw FragmentFileParseError(file, StringToWstring(e.what()));
		}
	...
	private:
		...
		// Constants for global instance ID:
		static constexpr std::string_view InstanceIDName = "g_instance_id_static";
		static constexpr std::string_view InstanceIDSemantic = "G_InstanceID";
		static constexpr std::string_view InstanceIDType = "uint";
		...
		enum WorkingFlag
		{
			...
			OutputInstanceID, // This node is contributing to instance ID computation, and should be outputted first
		...
		struct WorkingData
		{
			...
			Memory::SmallVector<size_t, 4, Memory::Tag::ShaderLinker> indirect_inputs;
		};
		...
		Memory::FlatMap<const DrawCalls::EffectGraphNode*, Memory::SmallVector<WorkingData, 128, Memory::Tag::ShaderLinker>, Memory::Tag::ShaderLinker> sub_working_data;
		...
		bool InlineUniform(Renderer::DrawCalls::EffectGraphParameter* param, char* buffer, size_t buffer_size) const
		{
			if (!parent->inline_uniforms)
				return false;

			int r = -1;
			switch (param->GetType())
			{
				case Renderer::DrawCalls::GraphType::Bool:			
					r = std::snprintf(buffer, buffer_size, "%s", (*param->GetParameter().AsBool()) ? "true" : "false"); 
					break;
				case Renderer::DrawCalls::GraphType::Int:			
					r = std::snprintf(buffer, buffer_size, "%d", *param->GetParameter().AsInt()); 
					break;
				case Renderer::DrawCalls::GraphType::UInt:
					r = std::snprintf(buffer, buffer_size, "%u", *param->GetParameter().AsUInt());
					break;
				case Renderer::DrawCalls::GraphType::Float:
					r = std::snprintf(buffer, buffer_size, "float(%.4g)", *param->GetParameter().AsFloat());
					break;
				case Renderer::DrawCalls::GraphType::Float2:
					r = std::snprintf(buffer, buffer_size, "float2(float(%.4g), float(%.4g))", param->GetParameter().AsVector()->x, param->GetParameter().AsVector()->y);
					break;
				case Renderer::DrawCalls::GraphType::Float3:
					r = std::snprintf(buffer, buffer_size, "float3(float(%.4g), float(%.4g), float(%.4g))", param->GetParameter().AsVector()->x, param->GetParameter().AsVector()->y, param->GetParameter().AsVector()->z);
					break;
				case Renderer::DrawCalls::GraphType::Float4:
					r = std::snprintf(buffer, buffer_size, "float4(float(%.4g), float(%.4g), float(%.4g), float(%.4g))", param->GetParameter().AsVector()->x, param->GetParameter().AsVector()->y, param->GetParameter().AsVector()->z, param->GetParameter().AsVector()->w);
					break;
				case Renderer::DrawCalls::GraphType::Spline5:
				{
					const auto packed = param->GetParameter().AsSpline5()->Pack();
					r = std::snprintf(buffer, buffer_size, "LoadSpline5(float4(float(%.4g), float(%.4g), float(%.4g), float(%.4g)), float4(float(%.4g), float(%.4g), float(%.4g), float(%.4g)), float4(float(%.4g), float(%.4g), float(%.4g), float(%.4g)), float4(float(%.4g), float(%.4g), float(%.4g), float(%.4g)), float4(float(%.4g), float(%.4g), float(%.4g), float(%.4g)))", 
									  ...
									  packed[3].x, packed[3].y, packed[3].z, packed[3].w,
									  packed[4].x, packed[4].y, packed[4].z, packed[4].w);
					break;
				}
				default:
					return false;
			...
			if (buffer_size > 0 && (r < 0 || size_t(r) >= buffer_size))
				throw ShaderCompileError("Failed to inline uniform");
		...
		std::string_view MakeCrossStageSemantic(const CrossStage& data)
		{
			...
			else if(data.index < data.node->GetOutputSlots().size())
			{
				const auto& link_name = data.node->GetOutputSlots()[data.index].Name();
		...
		std::string_view MakeNodeOutputName(const DrawCalls::EffectGraphNode* node, size_t index)
		{
			...
			else if(index < node->GetOutputSlots().size())
			{
				const auto& link_name = node->GetOutputSlots()[index].Name();
		...
		template<typename T>
		void ProcessInputNodes(const DrawCalls::EffectGraphNode* node, T&& func, bool indirect_inputs = false)
		{
			for (const auto& a : node->GetInputLinks())
				func(a.node);
			...
			if (const auto f = node_index.find(node); f != node_index.end())
				for (const auto& a : working_data[f->second].indirect_inputs)
					func(working_data[a].node);
		}

		template<typename T>
		void ProcessGroupChildNodes(const DrawCalls::EffectGraphNode* node, T&& func)
		{
			for (const auto& a : node->GetChildLinks())
				func(a.node);
		}
		...
		bool IsNodeAlive(const DrawCalls::EffectGraphNode* node, DrawCalls::Stage stage, Device::ShaderType shader, const Fragment* fragment)
		{
			...
			for (const auto& param : fragment->out)
				if (IsMachineSemantic(param->semantic))
					return true;

			for (const auto& param : fragment->inout)
				if (IsMachineSemantic(param->semantic))
					return true;
		...
		bool IsNodeInstanceIDOutput(const DrawCalls::EffectGraphNode* node, DrawCalls::Stage stage, Device::ShaderType shader, const Fragment* fragment)
		{
			for (const auto& param : fragment->out)
				if (param->semantic == InstanceIDSemantic)
					return true;
		...
		void BuildWorkingData()
		{
			...
			for (const auto& a : fragments)
			{
				...
				auto func = [&](
					Memory::SmallVector<const DrawCalls::EffectGraphNode*, 32, Memory::Tag::ShaderLinker>& in_stack,
					Memory::FlatMap<const DrawCalls::EffectGraphNode*, size_t, Memory::Tag::ShaderLinker>& in_node_index,
					Memory::SmallVector<WorkingData, 128, Memory::Tag::ShaderLinker>& in_working_data) -> bool
				{
					const auto stack_size = in_stack.size();
					const auto node = in_stack.back();
					ProcessInputNodes(node, [&](auto b)
					{
						if (in_node_index.find(b) != in_node_index.end())
							return;
						in_stack.push_back(b);
					});

					if (in_stack.size() != stack_size)
						return true;

					in_stack.pop_back();

					if (in_node_index.find(node) != in_node_index.end())
						return true;

					in_node_index[node] = in_working_data.size();
					in_working_data.emplace_back();

					if (auto f = parent->nodes.find(node->GetEffectNode().GetName()); f != parent->nodes.end())
						in_working_data.back().fragment = &f->second;
					else
						throw ShaderCompileError("Node " + node->GetEffectNode().GetName() + " was not found.");

					in_working_data.back().node = node;
					in_working_data.back().final_shader = shader;
					in_working_data.back().stage = a->GetStage();
					in_working_data.back().group_index = node->GetGroupIndex();
					in_working_data.back().graph_index = (int32_t)node->GetGraphIndex();
					in_working_data.back().flags.reset();

					if (node->GetEffectNode().IsInputType())
					{
						in_working_data.back().stage = node->GetStage();
						in_working_data.back().final_shader = in_working_data.back().stage == DrawCalls::Stage::VertexToPixel ? Device::ShaderType::VERTEX_SHADER : ShaderFromStage(working_data.back().stage);
					}

					if (IsNodeAlive(in_working_data.back().node, in_working_data.back().stage, in_working_data.back().final_shader, in_working_data.back().fragment))
						in_working_data.back().flags.set(WorkingFlag::IsAlive);

					if (IsNodeInstanceIDOutput(in_working_data.back().node, in_working_data.back().stage, in_working_data.back().final_shader, in_working_data.back().fragment))
						in_working_data.back().flags.set(WorkingFlag::OutputInstanceID);
				...
				do 
				{
					if (func(stack, node_index, working_data))
						continue;

					if (const auto node = working_data.back().node; node->GetEffectNode().IsGroup())
					{
						Memory::SmallVector<const DrawCalls::EffectGraphNode*, 32, Memory::Tag::ShaderLinker> group_stack;
						Memory::FlatMap<const DrawCalls::EffectGraphNode*, size_t, Memory::Tag::ShaderLinker> group_node_index;
						group_node_index[node] = 0;
						auto& sub_data = sub_working_data[node];

						ProcessGroupChildNodes(node, [&](auto b)
						{
							if (group_node_index.find(b) != group_node_index.end())
								return;
							group_stack.push_back(b);
							do
							{
								if (!func(group_stack, group_node_index, sub_data))
									if (sub_data.back().flags.test(WorkingFlag::OutputInstanceID))
										working_data.back().flags.set(WorkingFlag::OutputInstanceID); // Propagate flag to group parent node
		...
		{
			if (a.flags.test(WorkingFlag::OutputInstanceID) != b.flags.test(WorkingFlag::OutputInstanceID))
				return a.flags.test(WorkingFlag::OutputInstanceID);
		...
		void PropagateFlag(WorkingFlag flag)
		{
			for (size_t a = 0; a < working_data.size(); a++)
				if (working_data[a].flags.test(flag))
					index_stack.push_back(a);

			auto Push = [&](size_t idx)
			{
				if (idx >= working_data.size() || working_data[idx].flags.test(flag))
					return;

				working_data[idx].flags.set(flag);
			...
			while (!index_stack.empty())
			{
				...
				// Find and push previous output (connect extension points to previous stage)
				if (data.node->GetEffectNode().IsInputType())
				{
		...
		void FindIndirectInputs()
		{
			Memory::FlatMap<uint64_t, size_t, Memory::Tag::ShaderLinker> semantic_stack;
			for (size_t a = working_data.size(); a > 0; a--)
			{
				auto& node = working_data[a - 1];
				size_t num_indirect_inputs = 0;
				for (const auto& param : node.fragment->in)
					if (!param->semantic.empty())
						num_indirect_inputs++;

				for (const auto& param : node.fragment->inout)
					if (!param->semantic.empty())
						num_indirect_inputs++;

				node.indirect_inputs.reserve(num_indirect_inputs);

				for (const auto& param : node.fragment->out)
				{
					if (!param->semantic.empty())
					{
						const auto hash = HashSemantic(param->semantic);
						if (const auto f = semantic_stack.find(hash); f != semantic_stack.end() && f->second >= a)
						{
							working_data[f->second].indirect_inputs.push_back(a - 1);
							semantic_stack.erase(f);
						}
					}
				}
			...
			for (const auto& a : working_data)
			{
				...
				ProcessInputs(a.node, [&](const WorkingData& data, const DrawCalls::EffectGraphNode::Link& link)
				{
					if (data.final_shader == Device::VERTEX_SHADER && !data.flags.test(WorkingFlag::AnyShader))
					{
						...
						if (visited.find(hash) == visited.end() && link.output_info.index < link.node->GetOutputSlots().size())
						{
							visited.insert(hash);
							...
							const auto& link_name = link.node->GetOutputSlots()[link.output_info.index].Name();
		...
		void AddInputSemantic(const Device::ShaderParameter* a, bool allow_local_parameters)
		{
			...
			else  if(allow_local_parameters)
			{
				if (const auto f = local_parameters.find(hash); f != local_parameters.end() && f->second.type != a->type)
					throw ShaderCompileError("Semantic type does not match for semantic '" + std::string(semantic) + "'");

				if (semantic == InstanceIDSemantic && a->type != InstanceIDType)
					throw ShaderCompileError("Semantic type does not match for semantic '" + std::string(semantic) + "'");
		...
		void FindUniforms()
		{
			...
			auto func = [&](const WorkingData& data)
			{
				// Push uniforms
				...
				for (const auto& a : data.fragment->uniforms)
				{
					...
					if (!a->is_inline)
					{
						...
						else if (data.node->IsCustomParameter() || !InlineUniform(node_param->get(), nullptr, 0))
						{
			...
			ProcessActiveNodes([&](const WorkingData& data)
			{
				...
				if (data.node->GetEffectNode().IsGroup())
				{
					auto found = sub_working_data.find(data.node);
					if (found != sub_working_data.end())
					{
						for (const auto& sub_data : found->second)
							func(sub_data);
					}
				}
			});
		...
		void BuildDeclarations()
		{
			if (type == Device::ShaderType::COMPUTE_SHADER)
			{
				...
				shader << "#define GROUP_SIZE (GROUP_SIZE_X*GROUP_SIZE_Y*GROUP_SIZE_Z)\n";
			}
		...
		void BuildUniformLayout(const Memory::Span<const Declaration::Uniform>& uniforms, Device::UniformLayout& uniform_layout, const std::string_view& shader_prefix, const std::string_view& rate_prefix)
		{
			...
			shader << "\nstruct c" << rate_prefix << "\n{";

			unsigned padding_id = 0;
			auto write_padding = [&](size_t size, size_t padded)
			{
				if (size < padded)
				{
					shader << "\n\t";
					switch (padded - size)
					{
						case 4:	shader << "float"; break;
						case 8:	shader << "float2"; break;
						case 12:shader << "float3"; break;
						default: shader << "// " << unsigned(padded - size) << " bytes"; break;
					}

					shader << " padding" << (padding_id++) << ";";
				}
			};
			...
			for (const auto& a : uniforms)
			{
				...
				const auto size = size_t(a.size);
				const auto stride = GetUniformStride(a);

				shader << "\n";
				shader.WriteWithIndent(a.text, 1);
				write_padding(size, stride);
			}

			write_padding(layout_stride, uniform_layout.GetStride());
			...
			for (const auto& a : uniforms)
			{
				...
				{
					shader << "uniforms[" << shader_prefix << "_" << rate_prefix << "_uniforms_offset + (" << InstanceIDName << " / instance_count) * " << unsigned(uniform_layout.GetStride() / sizeof(simd::vector4)) << " + " << unsigned(pos) << "]";
					return shader;
				};
		...
		{
			shader << "static uint " << InstanceIDName << ";\n";
		...
		void BuildLocalParameters()
		{
			...
			for (const auto& a : sorted_parameters)
				if (a->semantic != InstanceIDSemantic)
		...
		void DeclareNodeParameterIn(const WorkingData& data)
		{
			...
			for (const auto& parameter : data.fragment->in)
			{
				...
				else if (!parameter->semantic.empty())
				{
					...
					{
						BeginParameter(f->second);
						if (parameter->semantic == InstanceIDSemantic)
							shader << " = " << InstanceIDName << ";";
						else
		...
		void DeclareNodeParameterOut(const WorkingData& data)
		{
			...
			for (const auto& parameter : data.fragment->out)
			{
				SHADER_MACRO_CHECK(active_macro, parameter->macro, shader);
				if (parameter->semantic == InstanceIDSemantic)
					shader << "\n\t\t#define " << parameter->name << " " << InstanceIDName;
				else
					shader << "\n\t\t" << MakeVariableDeclaration(parameter->type, parameter->name, false) << MakeVariableDefaultInitialisation(parameter->type) << ";";
			}
			END_SHADER_MACRO(active_macro, shader);
		...
		void DeclareNodeParameterInOut(const WorkingData& data)
		{
			...
			for (const auto& parameter : data.fragment->inout)
			{
				...
				else if (!parameter->semantic.empty())
				{
					...
					{
						if(parameter->semantic == InstanceIDSemantic)
							shader << "#define " << parameter->name << " " << InstanceIDName;
						else
		...
		void DeclareNodeParameterUniform(const WorkingData& data)
		{
			...
			for (const auto& a : data.fragment->uniforms)
			{
				...
				if (!a->is_inline)
				{
					...
					if (!data.node->IsCustomParameter() && InlineUniform(node_param->get(), buffer, std::size(buffer)))
						shader << buffer << ";";
					else
						shader << MakeParameterName(a->name, index) << ";";
				}
		...
		void AssignNodeInput(const WorkingData& data, AutoIncrementTable& autoincrement_table)
		{
			...
			{
				if (a.input_info.index >= data.node->GetInputSlots().size())
					continue;

				shader << "\n\t\t" << data.node->GetInputSlots()[a.input_info.index].Name();
				if (a.input_info.mask.size() != 0)
					shader << "." << a.input_info.mask;

				if (a.node->GetEffectNode().IsGroup())
				{
					if (data.node->GetParentId() == unsigned(-1))
						shader << " = " << MakeNodeOutputName(a.node, a.node->GetOutputSlots()[a.output_info.index].Name()) << "_out";
					else
						shader << " = " << MakeNodeOutputName(a.node, a.node->GetInputSlots()[a.output_info.index].Name()) << "_in";
				}
				else
				{
		...
		void DeclareGroupOutput(const WorkingData& data)
		{
			assert(data.fragment->is_group);
			assert(!data.node->GetOutputSlots().empty());
			for (const auto& output : data.node->GetOutputSlots())
			{
				shader << "\t" << MakeVariableDeclaration(FileReader::FFXReader::GetGraphTypeName(output.type), MakeNodeOutputName(data.node, output.Name()), false) << "_out";
				shader << MakeVariableDefaultInitialisation(FileReader::FFXReader::GetGraphTypeName(output.type)) << ";\n";
			}
		}

		void DeclareGroupInput(const WorkingData& data)
		{
			assert(data.fragment->is_group);
			for (const auto& input : data.node->GetInputSlots())
			{
				shader << "\t\t" << MakeVariableDeclaration(FileReader::FFXReader::GetGraphTypeName(input.type), MakeNodeOutputName(data.node, input.Name()), false) << "_in";
				shader << MakeVariableDefaultInitialisation(FileReader::FFXReader::GetGraphTypeName(input.type)) << ";\n";
			}
		}

		void AssignGroupInput(const WorkingData& data)
		{
			assert(data.fragment->is_group);
			for (const auto& a : data.node->GetInputLinks())
			{
				...
				shader << MakeNodeOutputName(data.node, data.node->GetInputSlots()[a.input_info.index].Name()) << "_in";
				if (a.input_info.mask.size() != 0)
					shader << "." << a.input_info.mask;
		...
		void AssignInitialGroupOutput(const WorkingData& data)
		{
			assert(data.fragment->is_group);
			assert(!data.node->GetOutputSlots().empty());
			for (const auto& output : data.node->GetOutputSlots())
			{
				...
				{
					shader << "\n\t\t";
					shader << MakeNodeOutputName(data.node, output.Name()) << "_out";
					shader << " = " << MakeNodeOutputName(data.node, output.Name()) << "_in";
					shader << ";";
				}
			}
		}

		void BeginGroup(const WorkingData& data)
		{
			assert(data.fragment->is_group);
			shader << "\n\t\t";
			if (!data.node->GetInputSlots().empty())
			{
				const auto& first = data.node->GetInputSlots()[0];
				if (first.Name() == DrawCalls::LoopCountInputName)
				{
					const auto loop_input = std::string(MakeNodeOutputName(data.node, first.Name())) + "_in";
					shader << "for(int index = 0; index < " << loop_input << "; ++index) {";
				}
				else
				{
					shader << "{";
		...
		void AccumulateGroupInput(const WorkingData& data)
		{
			assert(data.fragment->is_group);
			for (const auto& input : data.node->GetInputSlots())
			{
				...
				{
					shader << "\n\t\t";
					shader << MakeNodeOutputName(data.node, input.Name()) << "_in";
					shader << " = " << MakeNodeOutputName(data.node, input.Name()) << "_out";
					shader << ";";
				}
			}
		}

		void AssignFinalGroupOutput(const WorkingData& data)
		{
			assert(data.fragment->is_group);
			for (const auto& a : data.node->GetChildLinks())
			{
				if (a.input_info.index >= data.node->GetOutputSlots().size())
					continue;

				shader << "\n\t\t";

				shader << MakeNodeOutputName(data.node, data.node->GetOutputSlots()[a.input_info.index].Name()) << "_out";
				if (a.input_info.mask.size() != 0)
					shader << "." << a.input_info.mask;
		...
		void EndGroup(const WorkingData& data)
		{
			assert(data.fragment->is_group);
			shader << "\n\t\t}";
		}
		...
		void BuildNodes()
		{
			...
			ProcessActiveNodes([&](const WorkingData& data)
			{
				auto func = [&](const WorkingData& data)
				{
					...
					if (!data.fragment->is_group && data.fragment->content.empty() && data.fragment->inout.empty())
						return;
				...
				if (data.fragment->is_group)
				{
					NodeTitle(data);
					DeclareGroupOutput(data);
					shader << "\t{\n";
					DeclareGroupInput(data);
					AssignGroupInput(data);
					AssignInitialGroupOutput(data);
					BeginGroup(data);
					// TODO: Increase tab
					AccumulateGroupInput(data);
		...
		{
			BuildWorkingData();
			PropagateFlag(WorkingFlag::OutputInstanceID);
			FixNodeStages();
			SortWorkingData();
			FindIndirectInputs();
			PropagateFlag(WorkingFlag::IsAlive);
			MoveNodeShaderStage();
			CheckGroupIndex();
