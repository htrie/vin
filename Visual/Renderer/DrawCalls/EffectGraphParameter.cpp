...
namespace Renderer
{
	namespace DrawCalls
	{
		namespace ParameterUtil
		{
			...
			void Read(const JsonValue& param_obj, std::wstring& texture_path, Device::TexResourceFormat& format, bool& srgb)
			{
				...
				if (param_obj.HasMember(L"format"))
					...
					format = Device::TexResourceFormat::BC7;

				if (param_obj.HasMember(L"srgb"))
					srgb = param_obj[L"srgb"].GetBool();
			}

			void Save(JsonValue& param_obj, JsonAllocator& allocator, const std::wstring& texture_path, const Device::TexResourceFormat format, const bool srgb, const bool save_empty_path /*= false*/, const bool force_save_format /*= false*/)
			{
				...
				const auto default_format = Device::TexResourceFormat::BC7;
				if (format != default_format || force_save_format)
				{
		...
		void EffectGraphParameter::Copy(const EffectGraphParameter& param)
		{
			parameter = param.parameter;
			curve = param.curve;
			spline_colour = param.spline_colour;
			format = param.format;
			binding_parameter = param.binding_parameter;
			hash = param.hash;
		}
		...
		void EffectGraphParameter::SaveData(JsonValue& param_obj, JsonAllocator& allocator, const bool force_save /*= false*/) const
		{
			...
			case GraphType::TextureCube:
			{
				ParameterUtil::Save(param_obj, allocator, binding_parameter.texture_handle.GetFilename(), format, srgb, false, force_save);
				break;
			}
			case GraphType::Sampler:
			{
