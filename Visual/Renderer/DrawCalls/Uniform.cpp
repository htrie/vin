...
namespace Renderer::DrawCalls
{
	...
	Uniforms& Uniforms::AddDynamic(const uint64_t key, const Uniform&& value)
	{
		const auto offset = (uint16_t)storage.size();
		dynamic_entries.emplace_back(key, value.GetHash(), offset, value.GetType());
	...
	Uniforms& Uniforms::AddStatic(const uint64_t key, const Uniform&& value)
	{
		const auto offset = (uint16_t)storage.size();
		static_entries.emplace_back(key, value.GetHash(), offset, value.GetType());
		storage.resize(storage.size() + value.GetSize());
		assert(storage.size() < std::numeric_limits<uint16_t>::max());
		memcpy(&storage[offset], value.GetData(), value.GetSize());
		return *this;
	...
	{
		const auto offset = (uint16_t)storage.size();
		dynamic_entries.reserve(dynamic_entries.size() + uniforms.dynamic_entries.size());
		static_entries.reserve(static_entries.size() + uniforms.static_entries.size());
		storage.resize(storage.size() + uniforms.storage.size());
		assert(storage.size() < std::numeric_limits<uint16_t>::max());

		for (const auto& entry : uniforms.dynamic_entries)
			dynamic_entries.emplace_back(entry.key, entry.hash, offset + entry.offset, entry.type);
		for (const auto& entry : uniforms.static_entries)
			static_entries.emplace_back(entry.key, entry.hash, offset + entry.offset, entry.type);
	...
	void Uniforms::Append(Device::UniformInputs& inputs) const
	{
		for (const auto& entry : dynamic_entries)
			inputs.push_back(BuildInput(entry.type, entry.key, &storage[entry.offset]));
	...
	{
		for (unsigned i = 0; i < _uniforms.dynamic_entries.size(); ++i)
		{
			for (unsigned j = 0; j < dynamic_entries.size(); ++j)
			{
				if (dynamic_entries[j].key == _uniforms.dynamic_entries[i].key)
				{
					memcpy(&storage[dynamic_entries[j].offset], &_uniforms.storage[_uniforms.dynamic_entries[i].offset], DetermineSize(dynamic_entries[j].type));
					break;
				}
			}
		}
	...
	void Uniforms::Patch(uint64_t key, const Uniform&& value)
	{
		for (unsigned j = 0; j < dynamic_entries.size(); ++j)
		{
			if (dynamic_entries[j].key == key)
			{
				memcpy(&storage[dynamic_entries[j].offset], value.GetData(), value.GetSize());
				break;
			}
		}
	}
	...
	void Uniforms::Tweak(unsigned hash, const Uniform& value)
	{
		for (unsigned j = 0; j < dynamic_entries.size(); ++j)
		{
			if (dynamic_entries[j].hash == hash)
			{
				memcpy(&storage[dynamic_entries[j].offset], value.GetData(), value.GetSize());
				return;
			}
		...
		for (unsigned j = 0; j < static_entries.size(); ++j)
		{
			if (static_entries[j].hash == hash)
			{
				memcpy(&storage[static_entries[j].offset], value.GetData(), value.GetSize());
				return;
			}
		}
	}
	...
	unsigned Uniforms::Hash() const
	{
		...
		for (unsigned j = 0; j < dynamic_entries.size(); ++j)
		{
			hash = MergeHash(hash, (unsigned)(0xffffffff & dynamic_entries[j].key));
			hash = MergeHash(hash, (unsigned)((0xffffffff00000000 & dynamic_entries[j].key) >> 32));
		}

		for (unsigned j = 0; j < static_entries.size(); ++j)
		{
			hash = MergeHash(hash, (unsigned)(0xffffffff & static_entries[j].key));
			hash = MergeHash(hash, (unsigned)((0xffffffff00000000 & static_entries[j].key) >> 32));
		}
