...
namespace Renderer
{
	namespace DrawCalls
	{
		...
		EffectGraphInstance::EffectGraphInstance(const GraphDesc& graph_desc)
		{
			...
			auto func_head = [&](const unsigned index, const unsigned group_index)
			{
				const auto& head = graph->GetNode(index);
				const unsigned stage = head.GetStageNumber();

				auto func_node = [&](const EffectGraphNode& node)
				{
					if(processed_nodes.find(node.GetHashId()) != processed_nodes.end())
						return;
					...
					processed_nodes.insert(node.GetHashId());
				};

				head.ProcessChildren([&](const EffectGraphNode& node)
				{
					func_node(node);
					for (auto& link : node.GetChildLinks())
			...
			graph->ProcessOutputNodes(func_head);

			// bind drawdata from vertex output only nodes
			graph->ProcessOutputOnlyNodes(func_head);
		}
		...
		struct TempInputs
		{
			Memory::SmallVector<EffectGraphInstance, 16, Memory::Tag::DrawCalls> instances;
		...
		TempInputs SetupInputs(const EffectGraphDescs& graph_descs, EffectOrder::Value& effect_order)
		{
			...
			TempInputs result;
			result.instances.reserve(graph_descs.size());
