...
namespace Renderer
{
	namespace DrawCalls
	{
			...
			, palette(desc.palette)
		{
			...
			PROFILE_ONLY(uniforms.AddDynamic(DrawDataNames::PerformanceOverlay, simd::vector4(0));)

			uniforms.AddDynamic(DrawDataNames::SceneIndex, (unsigned)0);

		#ifdef ENABLE_DEBUG_VIEWMODES
			uniforms.AddDynamic(DrawDataNames::LightComplexity, simd::vector4(0));
		#endif
			uniforms.AddDynamic(DrawDataNames::LightIndices, simd::vector4(0));

			if (palette)
				uniforms.AddDynamic(DrawDataNames::AnimationMatricesIndices, simd::vector4(0));

			uniforms.AddDynamic(DrawDataNames::TrailVertexStride, unsigned(0));
			uniforms.AddDynamic(DrawDataNames::TrailVertexOffset, unsigned(0));
			uniforms.AddDynamic(DrawDataNames::ComputeThreadsX, instance_count_x);
			uniforms.AddDynamic(DrawDataNames::ComputeThreadsY, instance_count_y);
			uniforms.AddDynamic(DrawDataNames::ComputeThreadsZ, instance_count_z);

			uniforms.Append(uniform_inputs);
			uniform_input_layout = uniform_inputs.HashLayout();
		}
		...
		{
			const std::array<uint32_t, 14> ids = {
		...
		void DrawCall::SetVertexCount(const ::Trails::TrailDraw& trail_draw)
		{
			vertex_buffer.Reset();
			index_buffer.Reset();
		...
		{
			if (this->instance_count_x != instance_count_x || this->instance_count_y != instance_count_y || this->instance_count_z != instance_count_z)
			{
