...
namespace Renderer
{
	namespace DrawCalls
	{
		...
		private:
			...
			typedef Memory::FlatMap<unsigned, unsigned, Memory::Tag::EffectGraphNodes> ReplacementsMap;
			Memory::Object<EffectGraphNode>& FindReplacement(const EffectGraphNode& src, const ReplacementsMap& replacements);
			void CopyFlags(const EffectGraphHandle& other);
			void CopyInputs(Memory::Object<EffectGraphNode>& dest, const EffectGraphNode& src, const ReplacementsMap& replacements, const bool child_input);
			bool CopyNode(const EffectGraphNode& src, const EffectGraphNode& head, const unsigned group, const uint16_t graph_index, const unsigned parent_id, ConstEffectGraphNodes prev_heads, ReplacementsMap& replacements);
		};
