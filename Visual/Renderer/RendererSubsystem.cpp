...
#pragma pack(pop)
	...
	public:
		...
		const Shader::Desc& GetShaderDesc() const { return desc.shader_desc; }
	...
	public:
		...
		const Shader::Desc& GetShaderDesc() const { return desc.GetShaderDesc(); }
	...
	public:
		...
		{
			Memory::Lock lock(types_mutex);
			types.erase_if([&](const auto& type)
			{
				const auto& shader_desc = type->GetShaderDesc();
				for (auto& it : shader_desc.GetEffectGraphs())
				{
					if (it.second.to_view() == filename)
						return true;
				}
				return false;
			});
	...
	{
	#if defined(MOBILE)
		static const unsigned pipeline_cache_size = 256;
		static const unsigned type_cache_size = 512;
	#elif defined(CONSOLE)
		static const unsigned pipeline_cache_size = 1024;
		static const unsigned type_cache_size = 2048;
	#else
		static const unsigned pipeline_cache_size = 1024;
		static const unsigned type_cache_size = 2048;
	#endif
		...
		typedef Memory::Cache<unsigned, std::shared_ptr<Pipeline>, Memory::Tag::Pipeline, pipeline_cache_size> Pipelines;
		Memory::FreeListAllocator<Pipelines::node_size(), Memory::Tag::Pipeline, pipeline_cache_size> pipelines_allocator;
		Memory::Mutex pipelines_mutex;
		Pipelines pipelines;
		std::atomic_uint pipelines_job_count = 0;

		unsigned types_bucket_count = 0;
		std::array<TypesBucket<type_cache_size / 8>, 128> types_buckets;
		...
		{
			if (enable_throttling && (++type_touched_count >= type_cache_size * 4)) // Do not touch the cache if too many requests to avoid thrashing (causes infinite loading for instance).
				return Find(params);
		...
		Memory::SmallVector<std::shared_ptr<Pipeline>, pipeline_cache_size, Memory::Tag::Pipeline> SortPipelines()
		{
			Memory::SmallVector<std::shared_ptr<Pipeline>, pipeline_cache_size, Memory::Tag::Pipeline> to_sort;
	...
	public: // [TODO] Make private again.
		...
		void StartDevice()
		{
			...
			if (auto* window = Device::WindowDX::GetWindow())
			{
				...
				window->TriggerDeviceReset(device.get());
			}
		}
	...
	public:
		...
		{
			const unsigned backbuffer_width = device->GetBackBufferWidth();
			const unsigned backbuffer_height = device->GetBackBufferHeight();
		...
		#if defined(__APPLE__iphoneos)
			{
				...
				const float max_res =
					...
					0.3f;
				renderer_settings.resolution = Settings::Resolution(backbuffer_width * max_res, backbuffer_height * max_res);
			}
		#elif defined(CONSOLE) || defined(__APPLE__macosx)
			{
				renderer_settings.resolution = Settings::Resolution(backbuffer_width, backbuffer_height);
			}
		#else
			...
			{
				renderer_settings.resolution.width = std::min(renderer_settings.resolution.width, backbuffer_width);
				renderer_settings.resolution.height = std::min(renderer_settings.resolution.height, backbuffer_height);
			}
			else
			{
				renderer_settings.resolution = Settings::Resolution(backbuffer_width, backbuffer_height);
		...
		#endif
			LOG_DEBUG(L"[RENDER] backbuffer resolution = " << backbuffer_width << L" x " << backbuffer_height);
			...
			simd::vector2_int frame_size = simd::vector2_int(
				int(backbuffer_width * Device::DynamicResolution::Get().GetBackbufferToFrameScale().x + 0.5f),
				int(backbuffer_height * Device::DynamicResolution::Get().GetBackbufferToFrameScale().y + 0.5f)
			);
		...
		void Invalidate(const std::wstring_view filename)
		{
			LOG_INFO(L"[RENDER] Invalidate " << filename);
			...
			unsigned bucket_count, unsigned bucket_index)
		{
			...
			Entity::System::Get().ListProcess(sub_pass.blend_mode, [&](auto& templ, auto& batch)
			{
				...
				const auto max_batch_stride = draw_call_type->GetShaders().GetMaxStride(2, is_graphics);
				const auto max_batch_size = max_batch_stride == 0 ? ~size_t(0) : std::max(size_t(1), Device::UniformBuffer::GetMaxSize() / max_batch_stride);
				...
				{
					if (!enable_instancing || draw.sub_hash != first_draw.sub_hash || batch_uniform_inputs.size() >= max_batch_size)
					{
		...
		{
			render_graph.RenderDebugInfo(view_projection);
		}
	...
	void System::RenderDebugInfo(const simd::matrix & view_projection) { impl->RenderDebugInfo(view_projection); }
