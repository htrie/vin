...
namespace Visual
{
	void ReadRenderPasses(const std::wstring& buffer, MaterialPasses& out_passes, Objects::Origin origin /*= Objects::Origin::Client*/)
	{
		...
		for (auto& pass_obj : doc[L"passes"].GetArray())
		{
			...
			{
				new_pass.material = MaterialDesc(pass_obj[L"filename"].GetString());
			}
	...
	void SaveRenderPasses(std::wostream& stream, const Visual::MaterialPasses& material_passes, size_t indent /*= 1*/, Objects::Origin origin /*= Objects::Origin::Client*/)
	{
		...
		for (const auto& material_pass : material_passes)
		{
			...
			const auto filename = std::wstring(material_pass.material.GetFilename());
	...
	void ConvertGraphs(const GraphPasses& graph_passes, const std::wstring_view src_filename, MaterialPasses& material_passes, const ApplyMode apply_mode)
	{
		for (const auto& graph_pass : graph_passes)
		{
			...
			Visual::MaterialPass new_pass;
			new_pass.material = MaterialDesc(MaterialCreator(material_filename, graph_pass.second));
	...
	typedef Memory::FlatMap<unsigned, std::wstring, Memory::Tag::EffectGraphInstances> MatOverrides;
	void ConvertMaterialOverrides(const MatOverrides& mat_overrides, MaterialPasses& material_passes)
	{
		for (const auto& mat_override : mat_overrides)
		{
			...
			{
				if (pass.index == pass_index)
				{
					pass.material = MaterialDesc(mat_override.second);
					break;
				}
			}
			...
			{
				Visual::MaterialPass new_pass;
				new_pass.material = MaterialDesc(mat_override.second);
	...
	public:
		...
		{
			// TODO: conversion code, remove once conversion is done
			material_overrides[pass_index] = filename;
		}
		...
		void AddMiscEffectPack(const std::wstring& effect_name, MiscEffectType type, float delay) override final
		{
			...
			if (type == MiscEffectType::OnEnd)
				parent->SetEndEffectPack( row );
			else
				parent->SetBeginEffectPack( row, type == MiscEffectType::AfterDelay ? delay : 0.0f );
		}
	...
	void EffectPack::SetBeginEffectPack( const Loaders::MiscEffectPacks_Row_Handle row, const float delay )
	{
		if( play_epk_on_begin = row )
		{
			play_start_epk_after_delay = std::max( 0.0f, delay );
			if( row->GetEffectPack() && *row->GetEffectPack() )
				play_epk_on_begin_handle = row->GetEffectPack();
			if( row->GetEffectPackPlayer() && *row->GetEffectPackPlayer() )
				play_epk_on_begin_player_handle = row->GetEffectPackPlayer();
		}
		else
		{
			play_start_epk_after_delay = 0.0f;
			play_epk_on_begin_handle.Release();
			play_epk_on_begin_player_handle.Release();
		}
	}

	void EffectPack::SetEndEffectPack( const Loaders::MiscEffectPacks_Row_Handle row )
	{
		if( play_epk_on_end = row )
		{
			if( row->GetEffectPack() && *row->GetEffectPack() )
				play_epk_on_end_handle = row->GetEffectPack();
			if( row->GetEffectPackPlayer() && *row->GetEffectPackPlayer() )
				play_epk_on_end_player_handle = row->GetEffectPackPlayer();
		}
		else
		{
			play_epk_on_end_handle.Release();
			play_epk_on_end_player_handle.Release();
		}
	}

	void EffectPack::AddMaterialPass(const MaterialDesc& material, const unsigned pass_index, ApplyMode apply_mode, const bool apply_on_children)
	{
