...
namespace Device
{
	namespace GUI
	{
		...
		void ToolGUI::OnReloadSettings( const rapidjson::Value& settings )
		{
			ImGuiIO& io = ImGui::GetIO();

			const rapidjson::Value& imgui_settings = settings[ "imgui_settings" ];
...
#undef STYLE
		...
		void ToolGUI::OnLoadSettings(Utility::ToolSettings* settings)
		{
			...
			{
			#if defined(WIN32) && !defined(CONSOLE)
				const rapidjson::Value& window_settings = saved_settings["window_placement"];
				if (!window_settings.IsNull() && name != "path_of_exile")
				{
					auto& position = window_settings[ "position" ];
					auto& show_cmd = window_settings[ "show_cmd" ];
					if( position.IsArray() && position.GetArray().Size() == 4 && show_cmd.IsUint() )
					{
						WINDOWPLACEMENT window_placement;
						ZeroMemory( &window_placement, sizeof( WINDOWPLACEMENT ) );
						window_placement.length = sizeof( WINDOWPLACEMENT );
						window_placement.rcNormalPosition.left = position.GetArray()[ 0 ].GetInt();
						window_placement.rcNormalPosition.top = position.GetArray()[ 1 ].GetInt();
						window_placement.rcNormalPosition.right = position.GetArray()[ 2 ].GetInt();
						window_placement.rcNormalPosition.bottom = position.GetArray()[ 3 ].GetInt();
						window_placement.showCmd = show_cmd.GetUint();
						if( window_placement.showCmd == SW_SHOWMINIMIZED )
							window_placement.showCmd = SW_SHOWNORMAL;

						SetWindowPlacement( window->GetWnd(), &window_placement );
					}
				}
			#endif
			...
			OnReloadSettings( settings->GetDocument() );
		...
		void ToolGUI::OnSaveSettings(Utility::ToolSettings* settings)
		{
			...
			{
				const std::string save_name = name + std::string( "_common" );
				auto app_member = doc.FindMember( save_name.c_str() );
				if( app_member == doc.MemberEnd() )
				{
					doc.AddMember( Value().SetString( save_name.c_str(), allocator ), Value( kObjectType ), allocator );
					app_member = doc.FindMember( save_name.c_str() );
				}
				...
				if (strcmp(name, "path_of_exile") != 0)
				{
					...
					WINDOWPLACEMENT window_placement{};
					window_placement.length = sizeof( WINDOWPLACEMENT );
					if( window && GetWindowPlacement( window->GetWnd(), &window_placement ) )
					{
						Value normal_pos( kArrayType );
						normal_pos.PushBack( window_placement.rcNormalPosition.left, allocator );
						normal_pos.PushBack( window_placement.rcNormalPosition.top, allocator );
						normal_pos.PushBack( window_placement.rcNormalPosition.right, allocator );
						normal_pos.PushBack( window_placement.rcNormalPosition.bottom, allocator );
						window_settings.AddMember( "position", normal_pos, allocator );

						window_settings.AddMember( "show_cmd", Value().SetUint( window_placement.showCmd ), allocator );

						JsonUtility::ReplaceMember( app_member->value, std::move( window_settings ), "window_placement", allocator );
					}
				}
#endif
				app_member->value.EraseMember( "window_settings" );  // remove legacy settings

				JsonUtility::ReplaceMember( app_member->value, Value( show_help ), "show_help", allocator );
			}
			...
			#undef STYLE
				...
				JsonUtility::ReplaceMember( doc, std::move( imgui_settings ), "imgui_settings", allocator );
			}
		...
		void ToolGUI::OnRender( const float elapsed_time )
		{
			...
			if( environment_editor.open )
				environment_editor.OnRender( GetEnvironmentSettings(), elapsed_time, GetEnvironmentMixer(), GetPlayerObject() );
