...
namespace Utility
{
	...
	{
		{"BC1", "BC1 (Best for albedo OR albedo with alpha test, 4bpp)", Device::PixelFormat::BC1},
		{"BC4", "BC4 (Best for grayscale masks like metalness, 4bpp)", Device::PixelFormat::BC4},
		{"BC5", "BC5 (Best for two grayscale masks, 8bpp)", Device::PixelFormat::BC5},
		{"BC7Premult", "BC7 Premultiplied (Best for albedo with alpha blend, 8bpp)", Device::PixelFormat::BC7_Premultiplied},
		{"BC7", "BC7 (Best for combined normal/ao/gloss map, 8bpp)", Device::PixelFormat::BC7},
	...
	Device::TexResourceFormat StringToTexResourceFormat(const std::string& format_name)
	{
		...
		// remap old DXT naming
		if (format_name == "DXT1")
			return Device::TexResourceFormat::BC1;
		if (format_name == "DXT2" || format_name == "DXT4")
			return Device::TexResourceFormat::BC7_Premultiplied;
		if (format_name == "DXT3" || format_name == "DXT5")
			return Device::TexResourceFormat::BC7;
