...
namespace Utility
{
	...
	public:
		...
		ElementType* end() { return _data + _size; } // TODO: Check access bounds.
		...
		void Lock( Device::Handle<BufferType > buffer, const unsigned size, const Device::Lock lock );
		void Lock( Device::Handle<BufferType > buffer, const unsigned start, const unsigned size, const Device::Lock lock );
		void Unlock() { if( buffer ) { buffer->Unlock(); buffer.Reset(); } }
	...
	template< typename BufferType, typename ElementType >
	LockBuffer<BufferType, ElementType>::LockBuffer( Device::Handle<BufferType > buffer, const unsigned size, const Device::Lock lock )
	{
		Lock( std::move( buffer ), size, lock );
	}

	template< typename BufferType, typename ElementType >
	LockBuffer<BufferType, ElementType>::LockBuffer( Device::Handle<BufferType > buffer, const unsigned start, const unsigned size, const Device::Lock lock )
	{
		Lock( std::move( buffer ), start, size, lock );
	}

	template< typename BufferType, typename ElementType >
	void LockBuffer<BufferType, ElementType>::Lock( Device::Handle<BufferType > buffer, const unsigned size, const Device::Lock lock )
	{
	...
	template< typename BufferType, typename ElementType >
	void LockBuffer<BufferType, ElementType>::Lock( Device::Handle<BufferType > buffer, const unsigned start, const unsigned size, const Device::Lock lock )
	{
		this->buffer = std::move( buffer );
		this->_size = size;
