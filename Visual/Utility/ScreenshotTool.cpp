...
void ScreenshotTool::TakeScreenshot(Device::IDevice* device, Device::RenderTarget* rt, const std::wstring& screenshot_path, const bool add_postprocess_key, bool want_alpha)
{
	...
	try
	{
		...
		std::unique_ptr<Device::RenderTarget> shot = Device::RenderTarget::Create( "Screenshot", device, device->GetBackBufferWidth(), device->GetBackBufferHeight(), device->GetBackBufferFormat(), Device::Pool::SYSTEMMEM, true, false );

		rt->CopyToSysMem( shot.get() );

		if( want_alpha && device->GetBackBufferFormat() == Device::PixelFormat::A8R8G8B8 )
		{
			Device::LockedRect lock;
			shot->LockRect( &lock, Device::Lock::DEFAULT );
			for( unsigned int x = 0; x < device->GetBackBufferWidth(); x++ )
				for( unsigned int y = 0; y < device->GetBackBufferHeight(); y++ )
				{
