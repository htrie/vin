		...
		for( const auto* light : point_lights )
		{
			...
			for( unsigned i = 0; i < light_visualisation_density; ++i )
			{
				if( light_visualisation_density == 3 )
				{
					...
					UI::System::Get().DrawTransform( &points[ 0 ], ( DWORD )points.size(), &camera->GetViewProjectionMatrix(), color );
				}
				else
				{
			...
			if( direction.sqrlen() > 0.0f )
			{
				...
				UI::System::Get().DrawTransform( points, 2, &camera->GetViewProjectionMatrix(), color );
			}
