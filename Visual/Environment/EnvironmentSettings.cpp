...
namespace Environment
{
	...
	EnvironmentSettingsV2::EnvironmentSettingsV2()
	{
...

		{
			...
			settings_block.AddParam(EnvParamIds::Float::GroundScale, L"ground_scale", block_name, 0.004f);
...

		{
			...
			settings_block.AddParam(EnvParamIds::SoundParameterMap::SoundParameters, L"sound_parameters", block_name);
		}
	...
	LoadedEnvironment& EnvironmentSettingsV2::FetchData( const std::wstring& filename )
	{
		...
		try
		{
			...
			new_data->saved_data = new_data->env_data;
			new_data->undo_chain.push_back( new_data->env_data );
#endif
...
#if DEBUG_GUI_ENABLED
			it->second->saved_data = it->second->env_data;
			it->second->PushUndoState();
#endif
		}
	}
	...
	void EnvironmentSettingsV2::BlendData(const Data **datum, const float *weights, size_t data_count, BlendTypes blend_type, Data *dst_data)
	{
...
#if DEBUG_GUI_ENABLED
	void LoadedEnvironment::Undo()
	{
		if( current_undo_position > 0 )
		{
			env_data = undo_chain[ --current_undo_position ];
			has_unsaved_changes = env_data != saved_data;
		}
	}
	
	void LoadedEnvironment::Redo()
	{
		if( current_undo_position + 1 < undo_chain.size() )
		{
			env_data = undo_chain[ ++current_undo_position ];
			has_unsaved_changes = env_data != saved_data;
		}
	...
	void LoadedEnvironment::PushUndoState()
	{
		if( env_data != undo_chain[ current_undo_position ] )
		{
			undo_chain.resize( ++current_undo_position );
			undo_chain.push_back( env_data );
			has_unsaved_changes = env_data != saved_data;
		}
	}
#endif
