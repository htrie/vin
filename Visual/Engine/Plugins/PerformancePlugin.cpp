...
namespace Utility
{
	namespace
	{
		...
		static constexpr ViewModeInfo VIEW_MODE_INFOS[(unsigned)ViewMode::NumViewModes] = {
			/* DefaultViewmode		*/ { "Default", "", L"", Renderer::DrawCalls::Type::All, false },
			/* LightComplexity		*/ { "Light Complexity", "How many spot and directional lights affect the object:\n 0 lights : No color\n 1 light: Green\n 2 lights: Yellow\n 3 lights: Blue\n 4 lights: Red", L"Metadata/EngineGraphs/debug_light.fxgraph", Renderer::DrawCalls::Type::All, true },
			/* TiledLights  		*/ { "Tiled Lights", "Blue channel contains the number of point lights for that tile, times 64 (for better visualization)", L"Metadata/EngineGraphs/debug_tiled.fxgraph", Renderer::DrawCalls::Type::All, true },
			/* LightModel			*/ { "Lighting Model", "Red: SpecGlossPbrMaterial\nGreen: Anisotropy\nBlue: PhongMaterial", L"Metadata/EngineGraphs/debug_lighting_model.fxgraph", Renderer::DrawCalls::Type::All, true },
			/* DetailLighting		*/ { "Detail Lighting", "Shows objects with only the lighting on them, using the normal maps of the original material", L"Metadata/EngineGraphs/debug_detail_lighting.fxgraph", Renderer::DrawCalls::Type::All, false },
			/* LightingOnly			*/ { "Lighting Only", "Shows objects with only the lighting on them, without using the normal maps of the original material", L"Metadata/EngineGraphs/debug_lighting_only.fxgraph", Renderer::DrawCalls::Type::All, false },
			/* MipLevel				*/ { "Mip Level", "", L"Metadata/EngineGraphs/debug_miplevel.fxgraph", Renderer::DrawCalls::Type::All, true },
			/* Instancing			*/ { "Instancing", "Shades of HSV for each instance_id increment", L"Metadata/EngineGraphs/debug_instancing.fxgraph", Renderer::DrawCalls::Type::All, true },
			/* SceneIndex			*/ { "Scene Index", "Green: scene 0\nRed: scene 1\nBlue: scene 2\nYellow: scene 3", L"Metadata/EngineGraphs/debug_sceneindex.fxgraph", Renderer::DrawCalls::Type::All, true },
			/* Overdraw 			*/ { "Overdraw", "",L"Metadata/EngineGraphs/debug_overdraw.fxgraph", Renderer::DrawCalls::Type::All, true },
			/* DoubleSided			*/ { "Double Sided", "Orange: Double sided\nBlue: Single sided",L"Metadata/EngineGraphs/debug_doublesided.fxgraph", Renderer::DrawCalls::Type::All, true },
			/* ShaderComplexity		*/ { "Shader Complexity", "Shader cost from good to bad:\n Green -> Yellow -> Red", L"Metadata/EngineGraphs/debug_shadercomplexity.fxgraph", Renderer::DrawCalls::Type::All, true },
			/* PerformanceOverlay	*/ { "Performance Overlay", "", L"Metadata/EngineGraphs/debug_performance.fxgraph", Renderer::DrawCalls::Type::All, true },
			/* Polygons				*/ { "Polygons", "", L"Metadata/EngineGraphs/debug_polygons.fxgraph", Renderer::DrawCalls::Type::All, true },
			/* TrailWireframe		*/ { "Trail Wireframe", "", L"Metadata/EngineGraphs/debug_trail_wireframe.fxgraph", Renderer::DrawCalls::Type::Trail, true },
			/* DynamicCulling		*/ { "Dynamic Culling", "Color indicates particle density", L"Metadata/EngineGraphs/debug_dynamic_culling.fxgraph", Renderer::DrawCalls::Type::GpuParticle, true },
			/* Albedo				*/ { "Albedo", "", L"Metadata/EngineGraphs/debug_albedo.fxgraph", Renderer::DrawCalls::Type::All, false },
			/* Gloss				*/ { "Gloss", "", L"Metadata/EngineGraphs/debug_gloss.fxgraph", Renderer::DrawCalls::Type::All, false },
			/* Roughness			*/ { "Roughness", "", L"Metadata/EngineGraphs/debug_roughness.fxgraph", Renderer::DrawCalls::Type::All, false },
			/* Normal				*/ { "World Normal (Fit)", "Normals are fitted in the color space, so black = (-1, -1, -1) and white = (1, 1, 1)", L"Metadata/EngineGraphs/debug_normal.fxgraph", Renderer::DrawCalls::Type::All, false },
			/* Normal2				*/ { "World Normal (True)", "Normals are displayed as is (the color value equals the corresponding normal value, negative values are black)", L"Metadata/EngineGraphs/debug_normal2.fxgraph", Renderer::DrawCalls::Type::All, false },
			/* SurfaceNormal		*/ { "Surface Normal", "", L"Metadata/EngineGraphs/debug_surface_normal.fxgraph", Renderer::DrawCalls::Type::All, false },
			/* Specular				*/ { "Specular", "", L"Metadata/EngineGraphs/debug_specular.fxgraph", Renderer::DrawCalls::Type::All, false },
			/* Indirect				*/ { "Indirect Color", "", L"Metadata/EngineGraphs/debug_indirect.fxgraph", Renderer::DrawCalls::Type::All, false },
			/* SubSurface			*/ { "Subsurface", "", L"Metadata/EngineGraphs/debug_subsurface.fxgraph", Renderer::DrawCalls::Type::All, false },
			/* Glow					*/ { "Glow", "", L"Metadata/EngineGraphs/debug_glow.fxgraph", Renderer::DrawCalls::Type::All, false },
		};
		...
		static ImGuiTextFilter mode_filter;
	}
...
namespace Engine::Plugins
{
	...
	PerformancePlugin::PerformancePlugin()
		: Plugin(Feature_Visible | Feature_Settings)
	{
	#if DEBUG_GUI_ENABLED
	...
	void PerformancePlugin::OnRender(float elapsed_time)
	{
		...
		if (camera)
		{
			...
			Entity::System::Get().Process([&](auto& templ, auto& entity) 
			{
				...
				const auto stats = entity.draw_call->GetDrawStats();
				...
				final_stats.back().draw_call = entity.draw_call.get();
	...
	void PerformancePlugin::OnLoadSettings(const rapidjson::Value& settings)
	{
		...
		if (const auto f = settings.FindMember("reload_mode"); f != settings.MemberEnd() && f->value.IsString())
			if (auto mode = magic_enum::enum_cast<Engine::ReloadMode>(std::string_view(f->value.GetString(), f->value.GetStringLength())))
				reload_mode = mode.value();
	}
	...
	void PerformancePlugin::OnSaveSettings(rapidjson::Value& settings, rapidjson::Document::AllocatorType& allocator)
	{
		...
		const auto reload_mode_string = magic_enum::enum_name<Engine::ReloadMode>(reload_mode);
		settings.AddMember("reload_mode", rapidjson::Value().SetString(reload_mode_string.data(), (unsigned)reload_mode_string.size(), allocator), allocator);
	}
	...
	void PerformancePlugin::RenderUI()
	{
		...
		if (ImGui::BeginCombo("##ViewModes", buffer))
		{
			...
			Utility::mode_filter.Draw("##SearchFilter");
			bool close_popup = false;

			auto PassViewMode = [&](unsigned mode)
			{
				...
				if (name.empty() || !Utility::mode_filter.PassFilter(name.data(), name.data() + name.length()))
					return false;
			...
			size_t num_modes = 0;
			for (unsigned mode = 0; mode < unsigned(Utility::ViewMode::NumViewModes) && num_modes < 7; mode++)
			{
				if (!PassViewMode(mode))
					continue;

				num_modes++;
			}

			if (ImGui::BeginChild("##ScrollArea", ImVec2(0.0f, ImGui::GetTextLineHeightWithSpacing() * num_modes)))
			{
				for (unsigned mode = 0; mode < unsigned(Utility::ViewMode::NumViewModes); mode++)
				{
					...
					if (desc.length() > 0 && ImGui::IsItemHovered())
					{
						...
						ImGui::TextWrapped("%.*s", unsigned(desc.length()), desc.data());
		...
		ImGui::BeginDisabled(!Utility::VIEW_MODE_INFOS[unsigned(view_mode)].has_strength);

		float overlay = Renderer::Scene::System::Get().GetDebugOverlayStrength() * 100.0f;
		ImGui::SetNextItemWidth(item_width);
		if (ImGui::SliderFloat("##OverlayStrength", &overlay, 10.0f, 100.0f, "View Mode Intensity: %.1f%%", ImGuiSliderFlags_AlwaysClamp))
			Renderer::Scene::System::Get().SetDebugOverlayStrength(overlay / 100.0f);
		...
		{
			const auto names = magic_enum::enum_names<Engine::ReloadMode>();
			std::array<const char*, magic_enum::enum_count<Engine::ReloadMode>()> items;
			for (size_t i = 0; i < names.size(); ++i)
				items[i] = names[i].data();

			static int current = 0;
			ImGui::LabelText("##ReloadModeLabel", "Reload Mode");
			ImGui::Combo("##ReloadModeCombo", &current, items.data(), (int)items.size());

			Engine::ReloadMode old_reload_mode = reload_mode;
			if (const auto mode = magic_enum::enum_cast<Engine::ReloadMode>(current))
				reload_mode = mode.value();
			if (reload_mode != old_reload_mode)
			{
				Engine::System::Get().SetHotReload(reload_mode);
			}

			if (reload_mode == Engine::ReloadMode::Accumulate)
			{
				if (ImGui::Button("Reload Accumulated"))
					Engine::System::Get().ReloadAccumulated();
			}
		}
	#endif
	}
