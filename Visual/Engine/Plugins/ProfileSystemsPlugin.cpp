			...
			if (IsPoppedOut() || ImGui::BeginTable("##TableWrapper", 4, ImGuiTableFlags_NoBordersInBody | ImGuiTableFlags_NoSavedSettings | ImGuiTableFlags_SizingStretchSame | ImGuiTableFlags_NoPadOuterX))
			{
					...
					if (begin_system("Storage"))
					{
						...
						if (BeginSection("Download"))
						{
							...
							if (stats.download.enabled)
							{
								...
								ImGui::Text("Pre-Download Requests = %u", stats.download.pre_download_remaining_count);
								ImGui::Text("Pre-Download Count = %u", stats.download.pre_download_count);

								ImGui::Text("Download Count = %u", stats.download.download_count);
								...
								ImGui::Text("Memory Cache: %s", ImGuiEx::FormatMemory(buffer0, stats.download.memory_cache_size));
								ImGui::Text("Disk Cache: %s / %s", ImGuiEx::FormatMemory(buffer0, stats.download.disk_cache_size), ImGuiEx::FormatMemory(buffer1, stats.download.disk_cache_max_size));
							}
							else
							{
								ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.0f, 0.0f, 1.0f));
				...
				if (IsPoppedOut() || ImGui::TableSetColumnIndex(1))
				{
					...
					if (begin_system("Entity"))
					{
						...
						static size_t entity_peak_count = 0;
						entity_peak_count = std::max(entity_peak_count, stats.entity_count);
						static size_t template_peak_count = 0;
						template_peak_count = std::max(template_peak_count, stats.template_count);

						ImGui::Text("Entity: %u (peak %u)", unsigned(stats.entity_count), unsigned(entity_peak_count));
						ImGui::Text("Template: %u (peak %u)", unsigned(stats.template_count), unsigned(template_peak_count));
				...
				if (IsPoppedOut() || ImGui::TableSetColumnIndex(2))
				{
					...
					if (begin_system("GPU Particles"))
					{
...

						{
							...
							peak = std::max(peak, stats.num_used_slots);
							ImGuiEx::FormatMemory(buffer, stats.num_used_slots * stats.per_particle_mem);
							ImGuiEx::FormatMemory(buffer2, stats.max_particles * stats.per_particle_mem);
							ImGuiEx::FormatMemory(buffer3, peak * stats.per_particle_mem);

							ImGui::Text("Particles: %u / %u (Total: %u)", unsigned(stats.num_used_slots), unsigned(stats.max_particles), unsigned(stats.num_particles));
							ImGui::Text("Particles Memory: %s / %s (Peak: %s)", buffer, buffer2, buffer3);
						}

						{
							static size_t peak = 0;
							peak = std::max(peak, stats.num_allocated_emitters);
							ImGuiEx::FormatMemory(buffer, stats.num_allocated_emitters* stats.per_emitter_mem);
							ImGuiEx::FormatMemory(buffer2, stats.max_emitters * stats.per_emitter_mem);
							ImGuiEx::FormatMemory(buffer3, peak * stats.per_emitter_mem);

							ImGui::Text("Emitters: %u / %u (Total: %u, Visible: %u)", unsigned(stats.num_allocated_emitters), unsigned(stats.max_emitters), unsigned(stats.num_emitters), unsigned(stats.num_visible_emitters));
							ImGui::Text("Emitters Memory: %s / %s (Peak: %s)", buffer, buffer2, buffer3);
						}

						{
							static size_t peak = 0;
							peak = std::max(peak, stats.num_used_bones);
							ImGuiEx::FormatMemory(buffer, stats.num_used_bones * stats.per_bone_mem);
							ImGuiEx::FormatMemory(buffer2, stats.max_bones * stats.per_bone_mem);
							ImGuiEx::FormatMemory(buffer3, peak * stats.per_bone_mem);

							ImGui::Text("Bones: %u / %u (Total: %u)", unsigned(stats.num_used_bones), unsigned(stats.max_bones), unsigned(stats.num_bones));
							ImGui::Text("Bones Memory: %s / %s (Peak: %s)", buffer, buffer2, buffer3);
						}
						...
						ImGui::Text("Conversion Enabled: %s", GpuParticles::System::Get().GetEnableConversion() ? "true" : "false");
				...
				if (IsPoppedOut() || ImGui::TableSetColumnIndex(3))
				{
					...
					if (begin_system("Texture"))
					{
						...
						PLOT_TEXT("Frame Texture Upload Count", "Frame Upload Count: %u", stats.frame_upload_count, 32.0f);
						PLOT_TEXT("Frame Texture Upload Size", "Frame Upload Size: %lu", stats.frame_upload_size / Memory::MB , 256.0f);
