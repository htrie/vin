...
namespace Engine
{
	...
	public:
		...
		void OnReloadSettings(const rapidjson::Value& settings) override
		{
			for( const auto& plugin : plugins )
			{
				if( auto name = plugin.second->GetName(); !name.empty() )
				{
					...
					if( auto f = settings.FindMember( settings_name.c_str() ); f != settings.MemberEnd() && f->value.IsObject() )
						plugin.second->OnReloadSettings( f->value );
				}
			}
		}

		void OnLoadSettings(Utility::ToolSettings* settings) override
		{
		...
		void OnSaveSettings(Utility::ToolSettings* settings) override
		{
			if( !loaded_settings )
				return;
			...
			for (const auto& plugin : plugins)
			{
				if (auto name = plugin.second->GetName(); !name.empty())
				{
					...
					if( auto it = settings->GetDocument().FindMember( settings_name.c_str() ); it != settings->GetDocument().MemberEnd() )
						plugin_settings = it->value;
					plugin.second->OnSaveSettings(plugin_settings, settings->GetAllocator());

					if (plugin.second->FeatureNoDisable())
						settings->GetDocument().EraseMember(enabled_name.c_str());
					else
						JsonUtility::ReplaceMember( settings->GetDocument(), rapidjson::Value( plugin.second->IsEnabled() ), enabled_name, settings->GetAllocator() );

					if (plugin_settings.Empty())
						settings->GetDocument().EraseMember(settings_name.c_str());
					else
						JsonUtility::ReplaceMember( settings->GetDocument(), std::move( plugin_settings ), settings_name, settings->GetAllocator() );
				}
			}
		}
		...
		bool AddPlugin(Plugin* plugin)
		{
			...
			if( loaded_settings )
				LOG_WARN( L"Adding plugin " << StringToWstring( name ) << L"after loading settings. This may cause settings to be reset!" );
