...
#		define WATCH_ENGINE_FILES
#		include "Common/File/FileWatcherWin32.h"
#	endif
#endif
...
namespace Engine
{
	namespace
	{
		constexpr Version current_version = Version(1, 7, 0);
	...
	class Watcher
#if defined(WATCH_ENGINE_FILES)
		: public FW::FileWatchListener
#endif
	{
		bool accumulate = false;
		Memory::Vector<std::wstring, Memory::Tag::Profile> accumulated;

	#if defined(WATCH_ENGINE_FILES)
		FW::FileWatcherWin32 file_watcher;

		std::wstring ActionName(FW::Action action)
		{
			std::wstring res;
			if (action & FW::Action::Add) res += L"ADD ";
			if (action & FW::Action::Delete) res += L"DELETE ";
			if (action & FW::Action::Modified) res += L"MODIFIED ";
			return res;
		}

		virtual void handleFileAction(FW::WatchID watchid, const FW::String& dir, const FW::String& filename, FW::Action action) override
		{
			LOG_DEBUG(L"[WATCH] " << ActionName(action) << L" DIR " << dir << L" FILE " << filename);
			if (action == FW::Action::Modified)
			{
				const auto path = PathHelper::NormalisePath(dir + L"/" + filename);
				if (accumulate) { accumulated.push_back(path); }
				else { Engine::System::Get().ReloadAsset(path); }
			}
		}

		void MonitorFolder(const std::wstring_view name, bool recursive)
		{
			try
			{
				file_watcher.addWatch(name.data(), this, recursive);
				LOG_INFO(L"[WATCH] Monitoring " << name);
			}
			catch (FW::FileNotFoundException)
			{
				LOG_WARN(L"[WATCH] Failed to monitor " << name);
			}
		}
	#endif

	public:
		Watcher(bool accumulate)
			: accumulate(accumulate)
		{
		#if defined(WATCH_ENGINE_FILES)
			MonitorFolder(L"Art", true);
			MonitorFolder(L"Metadata", true);
		#endif
		}
		...
		void Flush()
		{
			for (auto& path : accumulated)
				Engine::System::Get().ReloadAsset(path);
			accumulated.clear();
		}
	};
	...
	void System::Init(const Settings& settings) // [TODO] Pass struct to constructors when all system are not singletons anymore.
	{
		...
		LOG_INFO(L"[ENGINE] Download directory: " << FileSystem::GetDownloadDirectory());
		...
		Shader::System::Get().SetInlineUniforms(settings.GetInlineUniforms());
		...
		Mat::System::Get().Init(settings.GetIgnoreTempFiles());
	...
	void System::SetPotato(bool enable)
	{
		...
		Mat::System::Get().SetPotato(enable);
	...
	void System::SetHotReload(ReloadMode mode)
	{
		LOG_INFO(L"[ENGINE] Hot-Reload: " << StringToWstring(magic_enum::enum_name<ReloadMode>(mode)));
		if (mode != ReloadMode::Off)
		{
			watcher = std::make_unique<Watcher>(mode == ReloadMode::Accumulate);
		}
		else
		{
			watcher.reset();
	...
	void System::Clear()
	{
		...
		LOG_INFO(L"[UI] Clear"); UI::System::Get().Clear();
		...
		LOG_INFO(L"[SCENE] Clear"); Mat::System::Get().Clear();
	...
	void System::OnCreateDevice(Device::IDevice* const device)
	{
		...
		UI::System::Get().OnCreateDevice(device);
	...
	void System::OnResetDevice(Device::IDevice* const device)
	{
		...
		UI::System::Get().OnResetDevice(device);
	...
	#if DEBUG_GUI_ENABLED
		Device::GUI::GUIResourceManager::GetGUIResourceManager()->OnResetDevice(device);
	#endif
	}
	...
	void System::FrameMove(const float elapsed_time, bool render_world, std::function<void()> callback_frame_loop, std::function<void()> callback_ui)
	{
		...
		Job2::System::Get().Add(Job2::Type::High, { Memory::Tag::Render, Job2::Profile::Render, [=]() {
			GpuParticles::System::Get().FrameMoveBegin(elapsed_time); // Before Entity::System::Get().Swap()
		...
		Job2::System::Get().Add(Job2::Type::High, { Memory::Tag::Material, Job2::Profile::Render, [=]() { Mat::System::Get().Update(elapsed_time); } });
		...
		Job2::System::Get().Add(Job2::Type::High, { Memory::Tag::Render, Job2::Profile::Render, [=]() { UI::System::Get().Update(elapsed_time); }});
		...
		Job2::System::Get().Add(Job2::Type::High, { Memory::Tag::Render, Job2::Profile::Particle, [=]() { GpuParticles::System::Get().FrameMoveEnd(); } });
		JOB_QUEUE_END();
	...
	void System::ReloadAccumulated()
	{
		if (watcher)
			watcher->Flush();
	}

	void System::ReloadAsset(const std::wstring_view filename)
	{
		if (EndsWithCaseInsensitive(filename, L".dds") ||
			EndsWithCaseInsensitive(filename, L".jpg") ||
			EndsWithCaseInsensitive(filename, L".ktx") ||
			EndsWithCaseInsensitive(filename, L".png"))
		{
			Texture::System::Get().Invalidate(filename);
		}
		else if (EndsWithCaseInsensitive(filename, L".fxgraph"))
		{
			EffectGraph::System::Get().Invalidate(filename);
			Renderer::System::Get().Invalidate(filename);
