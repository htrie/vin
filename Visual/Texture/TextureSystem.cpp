...
namespace Texture
{
	...
	public:
		...
		const std::wstring_view GetFilename() const { return desc.filename; }
	...
	public:
		...
		{
			Memory::Lock lock(levels_mutex);
			levels.erase_if([&](const auto& level) { return level->GetFilename() == filename; });
		}
...
#if defined(MOBILE)
	static const unsigned rate_limit = 4;
#elif defined(CONSOLE)
	static const unsigned rate_limit = 2; // Keep very low as to not hog CPUs (HDD is very slow, and we only have 6 true cores).
#else
	static const unsigned rate_limit = 8; // No higher than 16 to limit PCI-Express bandwidth (16 * 4k tex (16MB) = 256MB, @ 120Hz = 30GB, PCI-E 4.0 is 2GB/s per lane, so 32GB @ 16x).
#endif
		...
		void KickLevel(std::shared_ptr<Level> level)
		{
			if (enable_async && disable_async == 0 && level->IsAsync())
			{
				if (levels_job_count < rate_limit) // Rate-limit.
				{
					levels_job_count++;
					frame_upload_count++;
					frame_upload_size += level->GetTotalSize(device);
	...
	public:
		...
		void Invalidate(const std::wstring_view filename)
		{
			LOG_INFO(L"[TEXTURE] Invalidate " << filename);
		...
		void UpdateStats(Stats& stats)
		{
			...
			stats.frame_upload_count = frame_upload_count;
			stats.frame_upload_size = frame_upload_size;
