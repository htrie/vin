...
namespace Particles
{
	...
	public:
		...
		void SetGpuEmitterData(size_t index, const GpuParticles::EmitterTemplate &template_data) override
		{
			...
			for (auto& pass : parent->gpu_emitter_templates[index].render_passes)
			{
				pass.render_material.Fetch();
				pass.particle_mesh.Fetch();
			}

			parent->gpu_emitter_templates[index].update_material.Fetch();
			parent->gpu_emitter_templates[index].default_particle_mesh.Fetch();
		}
	};
	...
	ParticleEffect::ParticleEffect( const std::wstring &filename, Resource::Allocator& allocator)
	{
		...
		for (auto& emitter_template : emitter_templates)
		{
			...
			if (auto desc = GpuParticles::MakeDesc(emitter_template))
				GpuParticles::System::Get().Warmup(*desc);
		}
