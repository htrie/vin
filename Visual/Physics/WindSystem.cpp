...
namespace Physics
{
	...
	void RenderWindController(const simd::matrix & final_transform, Coords3f coords, AABB3f aabb, float duration, float elapsedTime)
	{
	...
	void RenderWakeVortex(const simd::matrix & final_transform, Coords3f coords)
	{
		Vector3f quad_vertices[6];
	...
	void RenderDebugParticle(const simd::matrix & final_transform, Vector3f point)
	{
		Vector3f quad_vertices[6];
	...
	void RenderDebugPrism(const simd::matrix & final_transform, Coords3f coords, Vector3f size, float angle, int segmentsCount, int color)
	{
	...
	void RenderDebugRing(const simd::matrix & final_transform, DebugRing ring, Coords3f coords, Vector3f scale)
	{
		RenderDebugPrism(final_transform, coords, Vector3f(ring.radius, ring.radius, ring.height) & scale, ring.angle, 10, ring.color);
	}
	void RenderDebugBox(const simd::matrix & final_transform, Coords3f coords, Vector3f size, int color)
	{
		int segmentsCount = 4;
		Vector3f offsets[4];
	...
	void WindSystem::RenderWireframe(const simd::matrix & final_transform)
	{
		for (size_t wind_index = 0; wind_index < explosion_wind_sources.GetElementsCount(); wind_index++)
		{
			...
			RenderWindController(final_transform, wind.controller.GetCoords(), wind.controller.GetAABB(), wind.controller.GetDuration(), wind.controller.GetElapsedTime());
			...
			for(size_t particle_index = 0; particle_index < wind.debugParticles.size(); particle_index++)
			{
				...
				RenderDebugParticle(final_transform, particle_pos);
			}
			for(size_t ring_index = 0; ring_index < wind.debugRings.size(); ring_index++)
			{
				RenderDebugRing(final_transform, wind.debugRings[ring_index], wind.controller.GetCoords(), Vector3f(1.0f, 1.0f, 1.0f));
			}
		}
		...
		for (size_t wind_index = 0; wind_index < wake_wind_sources.GetElementsCount(); wind_index++)
		{
			...
			for(int vortex_index = 0; vortex_index < WakeWindSource::vorticesCount; vortex_index++)
			{
				...
				RenderWakeVortex(final_transform, vortexCoords);
			}
			for(size_t particle_index = 0; particle_index < wind.debugParticles.size(); particle_index++)
			{
				Vector3f particle_pos = wind.debugParticles[particle_index].pos;
		...
		for (size_t wind_index = 0; wind_index < directional_wind_sources.GetElementsCount(); wind_index++)
		{
			...
			RenderDebugBox(final_transform, wind.controller.GetCoords(), wind.GetSize() & wind.controller.GetScale(), 0xff999999);
		}
	}
