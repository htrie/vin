...
public:
	...
	const Matrix2x2<T> GetInverted() const
	{
		...
		T invdet = fabs(det) > T(1e-7) ? T(1.0) / GetDeterminant() : T(1.0);
	...
	{
		Matrix2x2<T> Y_(zeroVector2f, zeroVector2f, zeroVector2f);
		Y_ = *this;
        Matrix2x2<T> Z_(Vector2<T>::xAxis(), Vector2<T>::yAxis(), Vector2<T>::zAxis());
		Matrix2x2<T> Y(zeroVector2f, zeroVector2f, zeroVector2f), Z(zeroVector2f, zeroVector2f, zeroVector2f);
