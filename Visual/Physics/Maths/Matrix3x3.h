...
public:
	...
	{
		Matrix3x3<T> Y_(zeroVector3f, zeroVector3f, zeroVector3f);
		Y_ = *this;
        Matrix3x3<T> Z_(Vector3<T>::xAxis(), Vector3<T>::yAxis(), Vector3<T>::zAxis());
		Matrix3x3<T> Y(zeroVector3f, zeroVector3f, zeroVector3f), Z(zeroVector3f, zeroVector3f, zeroVector3f);
  ...
  Vector3<T> GetColumn(int columnIndex)
  {
    return Vector3<T>(data[0][columnIndex], data[1][columnIndex], data[2][columnIndex]);
  }
