...
namespace Physics
{
	...
	void DeformableMesh::RenderWireframe(const simd::matrix & view_projection)
	{
		...
		for (size_t link_index = 0; link_index < links.size(); link_index++)
		{
			...
			UI::System::Get().DrawTransform(&line_verts[0], 2, &final_transform, 0xff336633);
		}
		...
		/*for (size_t bendIndex = 0; bendIndex < bend_constraints.size(); bendIndex++)
		{
			...
			UI::System::Get().DrawTransform(&line_verts[0], 2, &final_transform, 0xffaa33aa, 2.0f);
			...
			UI::System::Get().DrawTransform(&line_verts[0], 2, &final_transform, 0xffaaaa33, 2.0f);
