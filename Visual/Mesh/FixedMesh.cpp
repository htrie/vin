		...
		void SetHeader(const FixedMeshHeader& header) override
	{
		...
		void MergeSegments()
		{
			...
			while (merge_segment != segments.size())
			{
				...
				const bool can_merge = (segmentA.material.GetHash() == segmentB.material.GetHash()) && (segmentA.base_index + segmentA.index_count == segmentB.base_index);
				if (can_merge)
				{
		...
		void SetSegment(const SegmentData& data, uint32_t index) override
		{
			...
			segment.material = MaterialDesc(data.material_filename);
