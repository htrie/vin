...
namespace Mesh
{
	...
	void TileMeshInstance::SetMesh( const TileMeshChildHandle& subtile_mesh )
	{
		...
		normal_segment_visibility.resize( subtile_mesh->normal_section_material_indices.size(), true );
#endif
	}
	...
	void TileMeshInstance::UpdateWorldMatrix( const float x, const float y, const Orientation_t orientation, const int unit_height, const unsigned char tallwall_code )
	{
		...
		auto uniforms = Renderer::DrawCalls::Uniforms()
			.AddDynamic(Renderer::DrawDataNames::WorldTransform, world);

		if (ground_entity_id)
		{
			Entity::System::Get().Move(ground_entity_id, bounding_box, true, uniforms);
		}

		for (const MergedMeshSectionSegment& segment : segments)
		{
			Entity::System::Get().Move(segment.entity_id, bounding_box, true, uniforms);

			if(segment.wetness_entity_id)
			{
				Entity::System::Get().Move(segment.wetness_entity_id, bounding_box, true, uniforms);
			}
		}
	}
...
#undef MOVE_MEMBER
	...
	{
		const auto& mat = ConvertMaterialHandle( subtile_mesh.GetParent()->GetNormalSectionMaterials()[ segment_material_index ] );
		return ConvertMaterialDesc( material_overrides ? material_overrides->GetMaterial( mat ) : mat );
	}
...
//#pragma optimize( "", on )
	...
	void TileMeshInstance::ProcessSegments( const std::function<void(const TileMeshRawData::MeshSectionSegment& segment, const MaterialHandle& material )>& process, const std::function<void(const TileMeshRawData::MeshSectionSegment& mesh_segment)>& next) const
	{
		...
		for( auto segment = normal_section.segments.begin(); segment != normal_section.segments.end(); )
		{
			...
			const auto NextSegment = [&]( const bool call_next )
			{
...
#ifdef TILE_MESH_VISIBILITY
			if( !subtile_mesh.GetParent()->GetNormalMaterialVisibility()[ *segment_material_index ] || !normal_segment_visibility[ std::distance( normal_section.segments.begin(), segment ) ] )
			{
				NextSegment( false );
				continue;
			}
	...
	void TileMeshInstance::ForEachVisibleGroundSegment( const std::function<void( const TileMeshRawData::MeshSectionSegment& segment, const MaterialHandle& material )>& func ) const
	{
		if( subtile_mesh && subtile_mesh->mesh && subtile_mesh->mesh->HasGroundSection() )
		{
			const MaterialHandle no_material;
			for( const auto& segment : subtile_mesh->mesh->GetGroundSection().segments )
				func( segment, no_material );
		}
	}

	void TileMeshInstance::ForEachVisibleNormalSegment( const std::function<void( const TileMeshRawData::MeshSectionSegment& segment, const MaterialHandle& material )>& func ) const
	{
		if( subtile_mesh && subtile_mesh->mesh && subtile_mesh->mesh->HasNormalSection() )
		{
			MaterialHandle current_material;
			ProcessSegments( [&]( const TileMeshRawData::MeshSectionSegment& new_material_segment, const MaterialHandle& material )
			{
				current_material = material;
				func( new_material_segment, material );
			}, [&]( const TileMeshRawData::MeshSectionSegment& same_material_segment )
			{
				func( same_material_segment, current_material );
			} );
		}
	}
	...
	Entity::Desc TileMeshInstance::CreateGroundEntityDesc(const TileMeshRawData::MeshSection& mesh, const BoundingBox& bounding_box)
	{
		Entity::Desc desc;
		...
		case GroundCase2A:
			...
			desc
				...
				.AddObjectUniforms(Renderer::DrawCalls::Uniforms()
					.AddStatic(Renderer::DrawDataNames::Blend1UvTransformA, blend1_uv_transform_a)
					.AddStatic(Renderer::DrawDataNames::Blend1UvTransformB, blend1_uv_transform_b))
		...
		case GroundCase3A:
			...
			desc
				...
				.AddObjectUniforms(Renderer::DrawCalls::Uniforms()
					.AddStatic(Renderer::DrawDataNames::Blend1UvTransformA, blend1_uv_transform_a)
					.AddStatic(Renderer::DrawDataNames::Blend1UvTransformB, blend1_uv_transform_b)
					.AddStatic(Renderer::DrawDataNames::Blend2UvTransformA, blend2_uv_transform_a)
					.AddStatic(Renderer::DrawDataNames::Blend2UvTransformB, blend2_uv_transform_b))
				.AddObjectBindings({ { Device::IdHash(Renderer::DrawDataNames::UserBlendMask), blend_mask } });
			break;
		...
		case GroundCase1A1B:
			...
			desc
				...
				.AddEffectGraphs(ground_section_materials[1]->GetGraphDescs(), 1)
				.AddObjectUniforms(Renderer::DrawCalls::Uniforms()
					.AddStatic(Renderer::DrawDataNames::InvertGMask, default_g_mask))
		...
		case GroundCase2A1B:
			...
			desc
				...
				.AddObjectUniforms(Renderer::DrawCalls::Uniforms()
					.AddStatic(Renderer::DrawDataNames::Blend1UvTransformA, blend1_uv_transform_a)
					.AddStatic(Renderer::DrawDataNames::Blend1UvTransformB, blend1_uv_transform_b)
					.AddStatic(Renderer::DrawDataNames::InvertGMask, flags.test( SwapGMask ) ))
				.AddObjectBindings({ { Device::IdHash(Renderer::DrawDataNames::UserBlendMask), subtile_mesh.GetParent()->GetGroundMask() } });
			break;
	...
	Entity::Desc TileMeshInstance::CreateNormalEntityDesc(const TileMeshRawData::MeshSection& mesh, const MergedMeshSectionSegment& merged_segment, const BoundingBox& bounding_box)
	{
		Entity::Desc desc;
		...
		desc.SetType(Renderer::DrawCalls::Type::Water)
			...
			.SetLayers(scene_layers)
			.SetBlendMode(Renderer::DrawCalls::BlendMode::BackgroundGBufferSurfaces)
			.AddObjectUniforms(Renderer::DrawCalls::Uniforms().AddDynamic(Renderer::DrawDataNames::WorldTransform, world)
	...
	Entity::Desc TileMeshInstance::CreateWetnessEntityDesc(const TileMeshRawData::MeshSection& mesh, const MergedMeshSectionSegment& merged_segment, const BoundingBox& bounding_box)
	{
		Entity::Desc desc;
	...
	void TileMeshInstance::RecreateEntities()
	{
		...
		if (subtile_mesh->mesh->GetGroundSection().index_buffer && !ground_section_materials.empty())
		{
			...
			const auto desc = CreateGroundEntityDesc(mesh, bounding_box);
			Entity::System::Get().Create(ground_entity_id, desc);
		}
		...
		if (!normal_section.segments.empty())
		{
			...
			for (auto merged_segment = segments.begin(); merged_segment != segments.end(); ++merged_segment, ++current_merged_box)
			{
...

				{
					...
					const auto desc = CreateNormalEntityDesc(normal_section, *merged_segment, merged_segment->bounding_box);
					Entity::System::Get().Create(merged_segment->entity_id, desc);
				}
				#if WETNESS_PASS
					if(HasWetnessPass(*merged_segment))
					{
						merged_segment->wetness_entity_id = Entity::System::Get().GetNextEntityID();
						const auto desc = CreateWetnessEntityDesc(normal_section, *merged_segment, merged_segment->bounding_box);
						Entity::System::Get().Create(merged_segment->wetness_entity_id, desc);
					}
				#endif
			}
		}
