...
namespace Doodad
{
	...
	void DoodadBuffer::CreateEntity(Mesh& mesh)
	{
		...
		auto desc = Entity::Desc()
			...
			.AddObjectUniforms(Renderer::DrawCalls::Uniforms()
				.AddStatic( Renderer::DrawDataNames::WorldTransform, mesh.transform )
				.AddStatic( Renderer::DrawDataNames::FlipTangent, 1.0f )
				.AddStatic( Renderer::DrawDataNames::StartTime, -100.0f )
				.AddStatic( Renderer::DrawDataNames::GrassCenterPos, mesh.center_pos )
				.AddDynamic( Renderer::DrawDataNames::GrassDisturbLocationA, disturb_location_a )
				.AddDynamic( Renderer::DrawDataNames::GrassDisturbLocationB, disturb_location_b )
				.AddDynamic( Renderer::DrawDataNames::GrassDisturbTimes, disturb_times )
				.AddDynamic( Renderer::DrawDataNames::GrassDisturbForce, disturb_force ))
		...
		Entity::System::Get().Create(mesh.id, desc);
	}
	...
	void DoodadBuffer::MoveEntity(const Mesh& mesh)
	{
		if (mesh.id != 0)
		{
			const auto uniforms = Renderer::DrawCalls::Uniforms()
				.AddDynamic(Renderer::DrawDataNames::GrassDisturbLocationA, disturb_location_a)
				.AddDynamic(Renderer::DrawDataNames::GrassDisturbLocationB, disturb_location_b)
				.AddDynamic(Renderer::DrawDataNames::GrassDisturbTimes, disturb_times)
				.AddDynamic(Renderer::DrawDataNames::GrassDisturbForce, disturb_force);

			Entity::System::Get().Move(mesh.id, bounding_box, true, uniforms);
		}
	}
